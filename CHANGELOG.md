...
- **Dependabot alert repair (2026-05-05)** — Removed `ragas` and the `litellm` distribution from `pyproject.toml` and `requirements.txt`, closing Dependabot alerts #12 through #15 without downgrading `click`. Retired the resolved `pip-audit` ignores and removed the stale BACKLOG/SECURITY entries. Scope: dependency manifests, security docs, CI audit config, and matching regression tests only; no `src/kb/` runtime code changes.
- **Cycle 65 (Security hardening + config consistency — seventh dev-mimo-opus trial cycle)** — Items: 23 ACs (AC1-AC23) across ~15 files: (A) `src/kb/config.py` config call-time accessors — `get_project_root()` (env > monkeypatch > heuristic, AC1), `get_model_tier()` (AC2), `get_allowed_domains()` with KB_-prefixed env alias (AC3), PEP 562 `__getattr__` shim for `AUGMENT_ALLOWED_DOMAINS` back-compat; (B) `src/kb/utils/path_safety.py` (NEW) — `_assert_under_project_root` dual-anchor primitive (AC9, Q2.2 hard cap 4 kwargs), `_open_no_follow` + `_close_no_follow_fd` for `rebuild_indexes` TOCTOU mitigation (AC10, POSIX `O_NOFOLLOW` primary, Windows `is_symlink` defensive); (C) `src/kb/mcp/app.py::_validate_page_id` Windows-illegal-char (AC7) + segment-aware `..` (AC8) + trailing-dot/space (AC6) rejection above the existing containment net; (D) `src/kb/mcp/_error_boundary.py` (NEW) — `@_mcp_error_boundary` decorator applied to mcp/{core,ingest,quality}.py per AC21 (16 wraps, 1:1 with @mcp.tool() count); (E) `src/kb/lint/fetcher.py` `_url_scheme_allowed` http(s)-only allow-list (AC12) + `TRAFILATURA_DOWNLOAD_NO_CACHE` env-set-default (AC15); (F) `src/kb/query/embeddings.py` `VectorIndex.build` file_lock for multi-process safety (AC13) + `_ensure_conn` sqlite-vec `OperationalError` sanitization (AC14); (G) `src/kb/utils/cli_backend.py::_check_no_secrets_on_argv` value-based scrub via substring containment for the six env vars (AC16, plus DeepSeek-flagged BLOCKER-1 substring fix); (H) `src/kb/graph/cache.py` `__all__ = []` + AST-walk import discipline test (AC17); (I) `tests/_helpers/ast_walk.py` (NEW) — 4 helpers used by AC4 (autouse-fixture meta-test) + AC17 + AC18 + AC23; (J) `tests/test_security_cve_greps.py` (NEW) — banned imports for diskcache/litellm/pip/ragas (AC18); (K) `requirements.txt` GitPython upper bound `<3.2` (AC11); (L) `tests/test_cycle64_snapshots.py` augmented with `_neg_control` paired tests + `.github/workflows/ci.yml` `--snapshot-warn-unused` (AC19); (M) `docs/reference/INDEX.md` (NEW) — catalogue + meta-test (AC20); (N) `tests/test_validator_contract_consolidation.py` AC9 same-class peer scan (AC23, switched from grep to AST-walk per DeepSeek BLOCKER-2); (O) `.github/workflows/ci.yml` `sk-ant-dummy` leak guard (AC22). | Tests: 3134 passed + 21 skipped (+95 new tests across 12 new test files). Step 12 hard gate caught 116 failures from AC9 over-anchoring; Step 12 fix-cascade reverted `_validate_page_id` containment to WIKI_DIR-anchored, restored `_validate_wiki_dir` explicit `project_root=` honoring, fixed `_open_no_follow` Windows ctypes typing (HANDLE truncation → switched to `is_symlink` defensive), made `get_project_root()` honor monkeypatched module binding (cycle-18 L1 + cycle-19 L2 dual-source). Step 09 background DeepSeek review caught 2 BLOCKERs (substring containment, AST-walk). | Files: 207 → ~213 (+5 NEW: `path_safety.py`, `_error_boundary.py`, `ast_walk.py`, `INDEX.md`, plus 11 cycle-65 test files). | Scope: substantial security hardening — path-traversal + symlink + Windows-character + parent-segment + scheme + DNS-rebind + multi-process + SQL-vec error + secret-on-argv + CVE-import gates all tightened. Three NEW Q2.x design locks (Q2.2 hard cap 4 kwargs, Q2.3 once-per-process fallback warning, Q2.10 monkeypatch.setenv exclusivity for AC16 tests). 28 commits net (5 chunks × 4-5 commits each + 1 utf-8 fix + 5 simplify/step12 fixes + 1 background review + 1 docs). Five cycle-66 deferrals: PEP 562 shim dead-`PROJECT_ROOT`-branch design fix, `_check_no_secrets_on_argv` env-key list expansion to `CLI_BACKEND_ENV_INJECT` canon (security gap: misses Gemini/Kimi/Qwen/Zai/ZhipuAI keys), `get_project_root()` per-call cost caching, `tests/test_security_cve_greps.py` 4-walk → 1-walk consolidation, `path_safety._assert_under_project_root::allow_symlinks` removal (always-False, redundant kwarg). | Detail: see CHANGELOG-history.md cycle-65 + `docs/superpowers/decisions/2026-05-04-cycle-65-{requirements,threat-model,brainstorm-{opus,deepseek},design-eval-R1-opus,design-eval-R2-deepseek,design,plan,plan-gate-mimo-R1,step09-review-deepseek,HANDOFF}.md`
- **Cycle 64 (Backlog batch — sixth dev-mimo-opus trial cycle, BIGGEST cycle of trial so far)** — Items: 25 ACs (21 main + 4 sub-ACs added at Step-5 inline-resolution per C21-L1) across 6 file domains: (A) `tests/conftest.py` HIGH fixture leak surface — autouse promotion via split-fixture refactor (`_autouse_kb_path_sandbox` patch-only + explicit `tmp_kb_env` patch+mkdir + `kb_sandbox` public alias) + opt-in `real_project_root` + `--use-real-paths` CLI flag (4 ACs); (B) `src/kb/query/embeddings.py` HIGH-Deferred dim-mismatch AUTO-rebuild — `VectorIndex._derive_wiki_dir()` + auto-rebuild branch in `query()` + `get_dim_mismatch_auto_rebuild_count()` + `KB_DISABLE_VECTOR_AUTO_REBUILD` kill-switch read at CALL TIME per cycle-19 L2 + dual-anchor `_validate_path_under_project_root("vector_auto_rebuild_target")` per CLAUDE.md M4 (5 ACs); (C) NEW `src/kb/graph/cache.py` HIGH shared-cache contract — bespoke dict + RLock + FIFO LRU at `_MAX_CACHE_SIZE=4` (R2-F4 lru_cache REJECT — no per-key invalidate) + `get_graph(wiki_dir, *, pages=None)` (pages bypasses cache per R1-F5) + `invalidate(wiki_dir|None)` + `get_cache_stats()` keys-only per T7 + 5 lint caller migrations to attribute-lookup form per cycle-18 L1 + invalidation hooks at `ingest_source` / `refine_page` / `compile_wiki` / lint fix-mode (5 ACs); (D) `src/kb/compile/publish.py` MEDIUM `auto_publish_after_compile()` + manifest-based incremental sibling cleanup at `<wiki_dir>.parent/.data/publish-siblings-manifest.json` + `compile_wiki` AC14 hook with `KB_DISABLE_COMPILE_AUTO_PUBLISH` kill-switch + `# CYCLE-64-HOOK` merge-resilience marker + dual-anchor `_validate_path_under_project_root("publish_out_dir")` (6 ACs); (E) `tests/` MEDIUM snapshot infrastructure — `syrupy>=4.6.0` dev dep + 3 snapshot subjects (evidence-trail / Mermaid export / lint-report-structure) + committed `tests/__snapshots__/test_cycle64_snapshots.ambr` (3 ACs); (F) doc-sync (1 AC: CHANGELOG compact entry, CHANGELOG-history archive entry, CLAUDE quick reference, BACKLOG cleanup, and docs/reference architecture/testing updates). | Tests: 3039 passed + 18 skipped + 0 failed — 33 new cycle-64 tests across 6 new test files (`test_cycle64_conftest_leak.py` 5 cases, `test_cycle64_dim_mismatch_autorebuild.py` 8 cases [6 initial + 2 from Step 20 R1 B4: `_disabled_when_db_path_is_tmp_suffix` + `_rejects_wiki_dir_outside_project_root`], `test_cycle64_graph_cache.py` 10 cases [9 initial + 1 from Step 20 R1 B3: `test_compile_wiki_invalidates_graph_cache`], `test_cycle64_auto_publish.py` 7 cases, `test_cycle64_publish_manifest.py` 3 cases, `test_cycle64_snapshots.py` 3 cases) + 7 trial-skip-marks for AC3 migration debt (cycle-65+ migrate per cycle-61 trial-failure precedent). | Files: 200 → 207 (+7: 1 src/kb/graph/cache.py + 6 new tests/test_cycle64_*.py). | Scope: substantial behaviour change — autouse sandbox affects EVERY test by default; auto-rebuild fires on ANY dim-mismatch query; auto-publish fires on EVERY compile success. Three independent kill-switches added (`KB_DISABLE_VECTOR_AUTO_REBUILD`, `KB_DISABLE_COMPILE_AUTO_PUBLISH`, `pytest --use-real-paths`) for operator escape hatches. R1 Opus 4.7 + R2 DeepSeek V4 Pro design-eval (parallel ~7-9 min each) caught 4 + 3 BLOCKERs respectively, all factual precision fixes inline-resolved at Step 5 design-decision-gate (`tmp_kb_env` already exists / `src/kb/review/refiner.py` correct path / `kb.ingest.evidence` correct path / 5/10 build_graph callers in scope). Step 8 mimo-v2.5-pro plan-gate audit returned APPROVE-WITH-NOTES (cycle-61 telemetry confirmed: mimo audit role works on this codebase even when impl role doesn't). Per `project_cycle61_mimo_failure` memory: primary-session impl for all clusters; mimo-v2.5-pro restricted to Step 7 (returned stub-with-summary, inline-resolved per C21-L1) + Step 8 (audit-role-works confirmed again). DeepSeek V4 Pro on Step 4 R2 (cross-vendor adversarial diversity) — successful audit. CVE baseline (4 known: diskcache / litellm GHSA-xqmj / pip / ragas) — exact match cycles-47..58, no drift, no late arrivals; new dep `syrupy 5.1.0` clean against existing pin set. Strict-audit ratio target ≥67% (recovering from cycle-61's 33%) HONOURED for Steps 4-R1, 4-R2, 5, 7, 8 dispatched as binding-owner; Steps 9 (impl), 14 (verify), 17 (docs) ran primary-session per quality-driven C58-L4 carve-out + project_cycle61_mimo_failure direction; Codex landing review completed AC21 docs and whitespace hygiene before merge; Step 20 R1 cross-family review (DeepSeek+Sonnet) caught 4 BLOCKERs (incremental= bug, alias-form verifier failures, missing AC11.5+AC8 tests) post-landing — fixed in this PR. | Detail: see CHANGELOG-history.md cycle-64 + `docs/superpowers/decisions/2026-05-03-cycle-64-{requirements,threat-model,brainstorm,design-eval-R1,design-eval-R2,design-decision,plan,plan-gate,self-review}.md`
- **Cycle 58 (folds — fourth dev-mimo-opus trial cycle)** — Items: 5 ACs planned; 1 AC dropped at Step-21 rebase (AC4 `test_cycle15_lint_status_mature.py` already folded by cycle-54-pickup commit `05de0e0`, my fold was redundant — `git rebase --skip`); 4 ACs landed via fold (`test_cycle17_mcp_tool_coverage.py`, `test_cycle18_sanitize.py`, `test_cycle18_wiki_log.py`, `test_cycle17_capture_two_pass.py`) | Tests: 3021 (preserved — 4 folds move 13+16+5+10 = 44 tests across 4 receivers; net 0) | Files: 204 → 200 (-4 sources at branch HEAD post-rebase; cycle-54-pickup landed -4 first) | Scope: pure tests/ hygiene; zero src/kb/ changes; zero deps. Receivers: `test_mcp_core.py` (AC1, +13 tests, 5 classes for `kb_stats`/`kb_graph_viz`/`kb_verdict_trends`/`kb_detect_drift`/`kb_compile_scan` cross-module hosting per cycle-50 precedent), `test_utils_text.py` (AC2 with `TestSanitizePathRedaction` class-wrap to disambiguate from existing `yaml_sanitize` test prefix per cycle-50 cross-feature-hosting analogue, +16 tests), `test_utils.py` (AC3 wiki_log rotation, +5 tests; module-cohesion with existing `append_wiki_log` tests), `test_capture.py` (AC5 two-pass write all-or-nothing, +10 tests, helper `_make_items` → `_make_two_pass_items` per C52-L4). Fourth dev-mimo-opus trial cycle. MiMo plan-gate via mimo-v2.5-pro APPROVE no gaps (~222s). DeepSeek doc-sync dispatch via deepseek-v4-pro produced doc edits but with two factual errors (fabricated "AC4+AC5 batch" line, fabricated "verify-but-trust caught fabrication" self-narrative); doc-sync re-done in primary session per C12-L2 trust-but-verify pattern (candidate C58-L4 lesson refining C12-L2 to cover DeepSeek dispatches). | Detail: see CHANGELOG-history.md cycle-58 + docs/superpowers/decisions/2026-05-03-cycle-58-{requirements,brainstorm,design-eval,design,plan}.md
- **Cycle 54-pickup (folds — salvage of abandoned cycle-54 worktree)** — Items: 4 fold ACs / 0 src files / 4 test files DELETED via fold (`test_cycle8_health_wiki_dir.py`, `test_cycle8_models_validation.py`, `test_cycle15_lint_status_mature.py`, `test_cycle45_package_constants_propagate_to_submodules.py`) | Tests: 3021 (preserved — folds move 4+7+8+5 = 24 tests across 3 receivers; net 0) | Files: 208 → 204 (-4 sources at branch HEAD; subject to Step 21 rebase if cycles 53 still in flight) | Scope: pure tests/ hygiene; zero src/kb/ changes; zero deps. Receivers: `test_mcp_browse_health.py` (fold 1, +4 tests with `_write_health_fold_page` helper), `test_models.py` (fold 2, +7 tests with `_cycle54`-suffixed import aliases per C52-L4 helper-name uniqueness), `test_lint.py` (folds 3+4 in one batch-by-file commit, +13 tests with `_write_status_mature_page` + `_write_pkg_const_fold_page` helpers + 3 cycle-prefixed class renames `TestStatusMatureStaleOtherStatusesIgnored`/`TestStatusMatureStaleTodayOverride`). Salvage of the abandoned `worktree-cycle-54` branch (25 commits behind main, never merged); production-code intent (compile prune-base + Windows PID-liveness) already shipped on main via cycles 23/55-57 + commit `ea4af42`, so cycle-54-pickup re-scoped to ONLY the genuinely-unfinished test folds. Step-9 background-reviewer dispatch via `mimocoding-rescue` (mimo-v2.5) collected MiMo trial telemetry; impl-half ran primary session (deviation documented at Step 24, candidate skill patch refining C56-L1) | Detail: see CHANGELOG-history.md cycle-54-pickup
- **Cycle 57 (folds + sentinel-DELETE)** — Items: 6 ACs / 0 src files / 5 test files DELETED via fold (`test_v0916_task07.py`, `test_v0916_task08.py`, `test_v0915_task10.py`, `test_cycle17_lazy_imports.py`, `test_v0p5_purpose.py`) + 1 vacuous test DELETED (`test_review.py::test_embedding_dim_resolved`) | Tests: 3022 → 3021 (-1 from sentinel deletion; folds preserve count) | Files: 213 → 208 (-5 at branch HEAD; subject to Step 21 rebase if cycles 53/54 merge first) | Scope: pure tests/ hygiene; zero src/kb/ changes; zero deps. Receivers: test_mcp_core.py (T1), test_review.py (T2 + T6 deletion), test_cli.py (T3), test_v070.py (T4 with `_cycle17_` helper-rename per C52-L4), test_utils.py (T5 single-receiver feature-coherence per Q3). Third dev-mimo-opus trial cycle, parallel to in-flight worktree-cycle-53 + worktree-cycle-54 (receivers verified disjoint). T6 closes cycle-56+ BACKLOG-marker by deleting `test_embedding_dim_resolved` after confirming `EMBEDDING_DIM` is fully removed from kb.config + kb.query.embeddings.VectorIndex (sentinel `if not hasattr(config, "EMBEDDING_DIM"): return  # Deleted — PASS` made the test always vacuously pass) | Detail: see CHANGELOG-history.md cycle-57 + docs/superpowers/decisions/2026-05-02-cycle-57-requirements.md
