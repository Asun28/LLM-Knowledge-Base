...
- **Cycle 83 (ingest crash-atomicity — closes the data-loss half of Phase 4.5 HIGH R2)** — Items: 2 src files + 1 new test file. **The defect:** `_check_and_reserve_manifest` reserved the manifest slot with the BARE content hash, and that reservation happens BEFORE the first wiki page is written — ahead of the entire `_run_ingest_body` fan-out, not "between manifest-write and log-append" as the backlog recorded. A bare hash is indistinguishable from a completed ingest, so a crash anywhere across the body left `.data/hashes.json` asserting the source was fully ingested while the wiki held zero or partial pages; because `find_changed_sources` diffs stored-vs-current hash, that source was then skipped **permanently** on every later compile. Silent, with nothing telling the user to run `kb compile --full`. **The fix:** reserve as `in_progress:{hash}` — byte-identical to the producer string `compile_wiki` has written since cycle 25, so no new manifest format is introduced and the existing marker-aware consumers handle it unchanged. `find_changed_sources` already re-selects any value != current hash, so recovery needs **no new recovery pass**. Written inside the EXISTING `file_lock`, so no new lock target and the documented lock order is untouched. **Coverage:** all 6 `ingest_source` call sites funnel through this choke point, so `kb ingest` (CLI), `kb_ingest`/`kb_ingest_content` (MCP), `compile_wiki`, and `lint/augment/orchestrator.py:auto_ingest` are fixed by one change — previously only `compile_wiki` had marker protection, leaving 5 of 6 entry points exposed. **New `_claims_content` helper** encodes the three-way manifest vocabulary for duplicate detection: a bare hash and a live `in_progress:` both claim the content, `failed:` does not. Both halves are load-bearing — if markers did not claim, two threads ingesting distinct files with identical content would both write pages and reopen the Phase 4.5 Q_A RMW race; if `failed:` did claim, a prior failed attempt that wrote no pages would suppress a genuinely different source and leave that content in no page at all. **Design correction mid-cycle:** the architecture gate and threat model T8 disagreed on whether to prefix-strip before the duplicate comparison, and the initial decision (don't strip) was **overturned by the full-suite gate** — `test_duplicate_content_concurrent_ingest` failed, proving the dedup guarantee is shipped and load-bearing rather than theoretical. The three-way split is what falls out of honouring both objections; neither gate proposed it. Accepted residual: a hard-killed `in_progress:` marker does claim, so an identical-content file is skipped until the crashed source is retried — self-healing via the next compile, tracked as T8. **Exception path:** `except BaseException` downgrades the marker to `failed:{hash}`, wrapped in its own try/except so a rollback failure can never replace the caller's exception; the flag driving it is sentinel-initialised before the `try` per C33-L2. **Threat T2:** the swallow-all in `_check_and_reserve_manifest` moves `logger.debug` → `logger.warning`. **Separate live bug fixed** (found while writing the reselect test): `find_changed_sources` resolved the manifest default for `load_manifest`/`save_manifest` but passed the raw `manifest_path` to `file_lock`, so `None` raised `AttributeError: 'NoneType' object has no attribute 'with_suffix'` and killed the scan — reachable from the `kb_compile_scan` MCP tool on any call omitting `wiki_dir`; `compile_wiki` was unaffected (resolves the default earlier), which is why no existing test caught it. **Rejected:** the BACKLOG-proposed `.data/ingest_locks/<hash>.json` receipt design — it records that a crash happened but leaves `manifest[K] = H` intact, so the permanent skip is NOT fixed, while adding unbounded orphan accumulation and a second lock target that would need reconciling against the cycle-81 `page_lock`. **Suite-load finding:** the new tests initially ran a full `rebuild_vector_index` per ingest, and that extra I/O intermittently tipped two pre-existing timing-sensitive contradiction-concurrency tests past their `file_lock` timeout (~3 failures in 4 full runs, alternating between the two, never the same one twice in a row). Proven to be load rather than logic — the suite is green with the production changes in place and only the new test file excluded, and neither failing test calls any changed code. Fixed here by passing `_skip_vector_rebuild=True` in the fixture (nothing in the file exercises hybrid search); the underlying test fragility is filed to BACKLOG with a proposed event-driven fix. BACKLOG: HIGH entry rewritten to the residual (no rollback of wiki-side page writes, deliberately deferred); 6 new entries filed (Phase-2 `manifest_path` divergence, T8 liveness-aware dedup, T4 manifest value type check at the compiler read sites, T6 immortal markers, T12 parent-dir fsync). | Tests: 3461 → 3473 collected (+12 in `tests/test_cycle83_ingest_crash_atomicity.py`); ruff clean. | src/kb/ changes: 2 files (`ingest/pipeline.py`, `compile/compiler.py`). | Detail: see CHANGELOG-history.md cycle-83.
