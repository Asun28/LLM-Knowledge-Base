...
- `utils/io.py` `_purge_legacy_locks` — now runs lazily on first `file_lock` acquisition instead of at module import; prevents touching real `PROJECT_ROOT/.data/` at test-collection time (PR review R1 MAJOR)
...
- `ingest/evidence.py` `build_evidence_entry` vs `format_evidence_entry` — split restored: `build_*` stores byte-clean raw `source_ref`/`action`; `format_*` (new RENDER path) backtick-wraps pipes; `append_evidence_trail` now calls `format_*` so stored entries remain backward-compatible (PR review R1 MAJOR #28)
