...
- **Cycle 94 (freeze-and-fold — the FINAL pattern-set files + conftest stale-sandbox fix)** — Items: 5 fold sources → 41 source-subset placements across 18 canonical receivers; 176 tests moved VERBATIM (4 renames, all collision-driven with origin suffixes: 2 verdict tests + 1 `kb_create_page` test `_v070`, 1 `kb_create_page` test `_v090`), 5 versioned test files DELETED — this EMPTIES the Phase 4.5 R3 coverage-visibility pattern set (24 → 0 across cycles 80/90/92/93/94). Batch A (85): `test_v070.py` (56) + `test_v098_fixes.py` (29) → 12 receivers (15 placements) incl. the package-export block whole into `test_models.py` and the cycle-17 cold-boot block whole into `test_mcp_core.py`. Batch B (91): `test_v090.py` (43) + `test_v099_phase39.py` (33) + `test_v09_cycle5_fixes.py` (15) → 16 receivers (26 placements) incl. `test_review.py` (canonical `pair_page_with_sources` host) and `test_conftest_sandbox_guard.py`. **Plus one REAL FIX the fold exposed** — `tests/conftest.py` `_apply_kb_path_patches` mirror-rebind loop: the equality guard left a poisoning hole where a `kb.*` module FIRST-imported inside a sandboxed test captured that test's tmp paths at module level and could never be rebound (neither original-value nor anything the loop recognized); full runs were shielded only by collection-time imports, subset runs hit it via any lazy first-import (`kb.mcp.core` in the cycle-93 compile-scan test, `kb.mcp.browse` via the mcp_server smoke test). New second arm rebinds STALE-SANDBOX values (a Path/path-dict under this session's pytest basetemp but outside the current `tmp_path` is definitionally a leftover — no real repo path lives under basetemp). **The fold then exposed a SECOND leak in the same family**: KB_PROJECT_ROOT + `importlib.reload(kb.config)` tests (cycle-15 CLI containment) rebuild every derived config constant from the env at reload time, and teardown restores the env var but not the module attributes — constants deliberately NOT sandboxed (`TEMPLATES_DIR`, `RESEARCH_DIR` — repo-shipped artifacts) stayed bound to a dead tmp dir until the next reload, so the relocated `TestBuildExtractionSchema` comparison-template test read a nonexistent sandbox `comparison.yaml`. Fix: conftest snapshots every Path-valued `kb.config` constant at import (real values) and `_apply_kb_path_patches` repairs any stale non-sandboxed constant back to its real value each test. Both batches byte-verbatim audited (0 mismatches). **R1 Codex (2 P2, both fixed same-cycle)**: +3 self-contained regression tests in `test_conftest_sandbox_guard.py` deterministically seeding each repair arm (stale-sandbox module Path, mixed-stale `SOURCE_TYPE_DIRS` map, reload-leaked `TEMPLATES_DIR` restored to the REAL repo value) instead of relying on file-pairing order; and the v099 per-receiver counts corrected (duplicated `_make_wiki_page` helpers are not tests). | Tests: 3628 → 3631 (+3 R1 regression pins; the 176 folded tests are moves). | Files: 217 → 212 (−5). | src/kb/ changes: ZERO. | Detail: [history archive](CHANGELOG-history.md#2026-07-30--cycle-94-freeze-and-fold).
- **Cycle 93 (freeze-and-fold — the phase39N series + the last phase4_audit files)** — Items: 7 fold sources → 53 source-subset placements across 20 canonical receivers; 134 tests moved VERBATIM (1 helper renamed: `_make_rate_limit_error` → `_make_rate_limit_error_p393`, `test_llm.py` hosts an identical-named helper), 7 versioned test files DELETED. Batch A (54): `test_v0911_phase392.py` (9) + `test_v0912_phase393.py` (30) + `test_phase4_audit_{compile,observability,security}.py` (4+4+7) split across `test_refiner`/`test_mcp_browse_health`/`test_lint`/`test_compile`/`test_llm`/`test_query`/`test_ingest`/`test_graph`/`test_config`/`test_cli`/`test_evolve`/`test_mcp_core`. Batch B (80): `test_v0913_phase394.py` (39) + `test_v0914_phase395.py` (41) split across 18 receivers incl. `test_bm25`/`test_paths`/`test_utils_io`/`test_utils_text`/`test_models`/`test_feedback`/`test_lint_verdicts`/`test_lint_semantic`. Whole classes moved as units, majority-subject placement for mixed classes (each documented in CHANGELOG-history); both batches diff-verified byte-for-byte against `git show HEAD` sources (0 mismatches). Pattern set 12 → 5 (remaining: `test_v0{70,90}`, `test_v098_fixes`, `test_v099_phase39`, `test_v09_cycle5_fixes`). | Tests: 3628 collected (preserved — moves only); ruff clean. | Files: 224 → 217 (−7). | src/kb/ changes: ZERO. | Detail: [history archive](CHANGELOG-history.md#2026-07-30--cycle-93-freeze-and-fold).
- **Cycle 92 (freeze-and-fold — the complete remaining v0915 task series)** — Items: 6 fold sources → 19 receiver placements across 15 canonical files; 204 tests moved VERBATIM (1 class renamed: `TestGraphStatsDeterminism` → `TestGraphStatsDeterminism_t11`, receiver `test_graph.py` already hosts that name), 6 versioned test files DELETED. Batch A (91): `test_v0915_task03.py` (29, compiler + linker) → `test_compile.py`; `test_v0915_task06.py` (25) SPLIT → `test_lint.py` (14) + `test_lint_verdicts.py` (7) + `test_lint_semantic.py` (4); `test_v0915_task09.py` (37) SPLIT → `test_mcp_core.py` (35) + `test_mcp_browse_health.py` (2). Batch B (113): `test_v0915_task01.py` (57) SPLIT → `test_models.py` (37) + `test_utils_text.py` (9) + `test_utils_io.py` (7) + `test_ingest.py` (1) + `test_config.py` (3); `test_v0915_task08.py` (22) SPLIT → `test_evolve.py` (11) + `test_feedback.py` (11); `test_v0915_task11.py` (34) SPLIT → `test_query.py` (12) + `test_graph.py` (7) + `test_utils_io.py` (5) + `test_ingest.py` (4) + `test_compile.py` (4) + `test_lint_semantic.py` (2). Whole classes moved as units; per-source-per-receiver `# -- Cycle 92 fold from <file> (<subset>) --` banners; batch B verified byte-verbatim against `git show HEAD` copies after catching one heredoc-mangled line mid-fold. Completes the v0915 series (cycle 80 did task02/04/05/07): versioned pattern set 18 → 12 (remaining: 4 `test_v091N_phase39N`, `test_v0{70,90}`, `test_v098_fixes`, `test_v099_phase39`, `test_v09_cycle5_fixes`, 3 `test_phase4_audit_*`). | Tests: 3628 collected (preserved — moves only); ruff clean. | Files: 230 → 224 (−6). | src/kb/ changes: ZERO. | Detail: [history archive](CHANGELOG-history.md#2026-07-30--cycle-92-freeze-and-fold).
- **Cycle 91 (stale-marker hygiene + the Windows directory-durability won't-fix)** — Items: 4 ACs, 2 src files + 1 new test file + 1 reference doc. Two MEDIUM backlog closures, both operational. **AC01** `compile_wiki`'s premarker gains a birth-timestamp third segment (`in_progress:{pre_hash}:{unix_ts}`); every consumer checks `startswith("in_progress:")` or deletes whole entries, so the format extension is invisible to them (one cycle-25 test compared "everything after the first colon" and was made segment-aware). New `_marker_age_days` returns `None` — "age unknown" — for legacy two-segment, unparseable, and future-dated (clock-skew) values rather than guessing, per the `get_prompt_version` legacy-default convention. **AC02** AC7's stale-marker warning annotates each key with its age and caps the per-key enumeration at `STALE_MARKER_WARNING_CAP=10` with an "and M more" suffix — the full COUNT is always reported; only the listing truncates. This supersedes cycle-25's Step-8 full-enumeration choice: cycle-83's T6 showed the unbounded log line is its own failure mode. **AC03** `clear_stale_markers()` + `kb compile --clear-stale-markers` — the explicit operator remediation AC7's "operator decides" contract always implied but never provided (previously the only clear path was full `kb rebuild-indexes`). AUTO-deletion stays rejected (cycle-25 Q10: races a live compile); the flag's help text warns that clearing during an in-flight compile discards that run's live marker — the blast radius is one lost AC7 warning, never source data. **AC04** the cycle-89 residual is CLOSED AS WON'T-FIX with the verification written into `docs/reference/error-handling.md`: no SUPPORTED unprivileged Win32 primitive flushes a DIRECTORY, so completed DELETES cannot be made durable-on-return — renames are the exception via their own `MOVEFILE_WRITE_THROUGH` flag (`FlushFileBuffers` rejects directory handles, `NtFlushBuffersFileEx` documents its sync flag as invalid on directories, `DeleteFileW` has no write-through flag; TxF exists but is deprecated by Microsoft with conditional power-fault guarantees and is REJECTED deliberately; the volume-handle flush needs admin). The Windows delete-resurrection risk is ACCEPTED and deliberately unreported per-call. Renames ARE covered (`MOVEFILE_WRITE_THROUGH`, cycle 87); NTFS journaling gives deletes crash-consistency, not durability-on-return. Reviewers re-proposed reporting this gap in cycles 87, 88, AND 89 — the doc entry exists so the fourth proposal gets a pointer instead of a re-derivation. **R1 Codex fixes (4 P1)**: `clear_stale_markers` now STRICT-loads the manifest and raises `CompileError` on corruption instead of inheriting `load_manifest`'s self-heal-to-`{}` (which would have reported "no markers" against a corrupt-but-recoverable file and let the compile overwrite it; a MISSING manifest still returns `[]`); the CLI clear runs inside its own `_error_exit` boundary, fail-fast (no compile after a failed clear); `_marker_age_days` rejects non-finite timestamps (`float("nan")` passed both ordered comparisons and rendered "age nand"); the docs claim softened to "no SUPPORTED unprivileged primitive" (TxF exists but is deprecated with conditional power-fault guarantees — deliberately rejected, and the Windows delete-resurrection risk is explicitly accepted). | Tests: 3608 → 3628 (+20: 7 AC01, 4 AC02, 9 AC03/R1). | Detail: [history archive](CHANGELOG-history.md#2026-07-30--cycle-91-stale-marker-hygiene).
- **Cycle 90 (freeze-and-fold — the complete recovered cycle-59/62/63 12-file set)** — Items: 12 fold sources → 8 receivers; 69 tests moved VERBATIM (1 helper renamed: `_make_page` → `_make_page_p4q` in the `test_phase4_audit_query` block, receiver had a same-named method), 12 versioned test files DELETED. Folds: `test_v4_11_cli.py` (5, CLI query formats) → `test_cli.py`; `test_v4_11_markdown.py` (8, markdown adapter) + `test_phase4_audit_query.py` (5, `_build_query_context`) → `test_query.py`; `test_v4_11_mcp.py` (7, `kb_query` format param) → `test_mcp_core.py`; `test_v5_augment_config.py` (2, augment config constants) → `test_config.py`; `test_v5_autogen_prefixes.py` (3, `AUTOGEN_PREFIXES` skip) + `test_stub_detection_v094.py` (6, `check_stub_pages`) → `test_lint.py`; `test_v5_verdict_augment_type.py` (3, augment verdict type) → `test_lint_verdicts.py`; `test_v5_lint_augment_manifest.py` (7, manifest state machine) + `test_v5_lint_augment_rate.py` (7, rate limiter) → `test_lint_augment_split.py`; `test_ingest_fixes_v092.py` (8, `_update_existing_page` + `_format_ingest_result`) + `test_phase4_audit_ingest.py` (8, hashing/data-integrity) → `test_ingest.py`. This lands the COMPLETE 12-file set recovered 2026-07-23 from the abandoned cycle-59/61/62/63 branches — the `archive/cycle-62` tag served as placement reference only; every fold was done fresh against current receivers (the archived diffs predate cycles 65-89 and no longer apply). Advances Phase 4.5 HIGH coverage-visibility cadence: versioned `test_v0*`/`test_phase4_audit` files 20 → 18 (`phase4_audit_{ingest,query}` leave that set; the other 10 deleted files were outside its naming pattern). | Tests: 3608 collected (preserved — moves only; 3562 passed / 29 skipped / 17 xfailed full local run); ruff clean. | Files: 241 → 229 (−12). | src/kb/ changes: ZERO. | Detail: [history archive](CHANGELOG-history.md#2026-07-30--cycle-90-freeze-and-fold).
- **Cycle 89 (barrier tri-state — the shared root cause of two cycle-88 review findings)** — Items: 2 ACs, 2 src files + 1 new test file. Cycle 88's review produced the same finding twice from two model families approaching from opposite ends: R1 DeepSeek said the Windows rollback claims a durability it does not have, R2 Codex said the POSIX tolerated-errno path does the same. Both proposed having callers REPORT a missing barrier, and neither noticed that `_fsync_parent_dir` returned `None` in all three cases — the information needed to report it did not exist. Cycle 88 rejected both remedies for consistency and filed the root cause; this cycle closes it. **AC01** `_fsync_parent_dir` returns `BarrierResult.{FLUSHED,UNSUPPORTED,SKIPPED_PLATFORM}`; a genuine storage failure (`EIO`/`ENOSPC`) still RAISES, so there are three values and not four. `_dir_fsync_supported()` is added as the platform seam per C86-L3 — a `skipif`-guarded branch is untested on whichever platform the developer is not using, which is how the Windows gap survived cycle 86; it is kept separate from `_use_windows_write_through()` because the two answer different questions and could diverge. Additive: `durable_replace` and `durable_rename` ignore the return value and are behaviourally unchanged, pinned by test. **AC02** `capture._finish_rollback` consumes it, and this is where the third value earns its keep: `UNSUPPORTED` is unusual and filesystem-specific so it emits `BARRIER_UNSUPPORTED_MARKER`, while `SKIPPED_PLATFORM` is constant on Windows and stays SILENT — surfacing it would ride along on every capture failure and train the reader to ignore it, the crying-wolf failure cycle 88 rejected twice. A bool could not express that split. The note is a SEPARATE suffix from `ROLLBACK_INCOMPLETE_MARKER` and both can appear on one error: they make different claims ("state UNKNOWN, go look" vs "state known, not yet durable"), and merging them would undo the cycle-88 decision. Tests: 3537 → 3562 (+26 incl. 1 platform-skipped); the AC01 tests drive BOTH platform branches on either OS via the new seam, cutting 11 skips to 1. Scope: closes the API half of the filed residual; the remaining half is that Windows has no directory-flush primitive at all, rewritten in BACKLOG and likely won't-fix. Detail: [history archive](CHANGELOG-history.md#2026-07-29--cycle-89-barrier-tri-state).
