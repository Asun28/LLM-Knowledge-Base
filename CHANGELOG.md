...
- **Cycle 62 (folds — 20-file batch)** — Items: 20 fold sources / 0 src files / 20 test files DELETED via fold (97 tests moved into 8 existing receivers: `test_query.py`, `test_ingest.py`, `test_lint.py`, `test_compile.py`, `test_config.py`, `test_lint_verdicts.py`, `test_cli.py`, `test_mcp_core.py`) | Tests: 3022 (preserved — 97 tests folded, 97 land in receivers, net 0) | Files: 200 → 180 (-20 sources) | Scope: pure tests/ hygiene; zero `src/kb/` changes; zero deps. Source baseline `97 passed`; targeted receiver batches `26+61+111+133 passed`; full Windows local `3010 passed + 12 skipped`; ruff format/check clean; pip-audit reports the same 4 pre-existing baseline advisories. | Detail: see CHANGELOG-history.md cycle-62 + docs/superpowers/decisions/2026-05-03-cycle-62-{requirements,design,plan}.md
