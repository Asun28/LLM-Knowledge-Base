...
*Last reviewed: 2026-08-17 (dependency-security sweep — Dependabot alerts #90–#95 closed by pin bumps: `aiohttp` 3.14.1 → 3.14.3 (CVE-2026-59881 unnegotiated compressed WebSocket frames, CVE-2026-69243 WebSocket-upgrade request smuggling, CVE-2026-69244 out-of-bounds heap read in the C response parser), `cryptography` 48.0.1 → 50.0.0 (CVE-2026-69247 PKCS#7 EnvelopedData Bleichenbacher oracle), `h2` 4.3.0 → 4.4.1 (CVE-2026-71554 duplicate-Host request smuggling), `pymdown-extensions` 11.0.0 → 11.0.1 (CVE-2026-67422 ReDoS in the caret/tilde/betterem/magiclink inline processors). Two co-bumps forced by the resolver rather than by their own advisories: `pyOpenSSL` 26.2.0 → 26.4.0 (26.2.0 declares `cryptography<49`, so the cryptography fix is uninstallable without it) and `hpack` 4.1.0 → 4.2.0 (`h2` 4.4.1 declares `hpack>=4.2,<5`). Separately, the `GitPython` floor was raised `>=3.1.51` → `>=3.1.58`, closing 7 advisories (GHSA-3f7w-8rr8-f37f, GHSA-p538-c434-8v24, GHSA-9rj7-rf2p-w77r, GHSA-4gmw-gg2m-w46p, GHSA-hh9p-6wh2-4mfc, GHSA-wvpp-8hx9-p66j, GHSA-jm78-9fvv-mhgr) that `pip-audit` flagged against the resolved 3.1.55 but Dependabot had NOT filed — the strict CI audit would have failed on them regardless of the Dependabot queue. Local `pip-audit` now reports ZERO findings, `pip check` is clean, and the accepted-advisory table stays EMPTY. Cycle 79 earlier: nltk 3.9.4 → 3.10.0 — GHSA-p4gq-832x-fm9v / CVE-2026-54293 advisory gained its fixed-version stamp (3.10.0), closing the last tracked upstream advisory and Dependabot alert #63; local `pip-audit` now reports ZERO findings and `pip check` is clean. Cycle 76 earlier: dspy + 6 orphan pins removed → diskcache CVE-2025-69872 eliminated from the tree, accepted-advisory table now EMPTY, CI pip-audit runs exception-free, CI pip check strict. Cycle 75: joserfc 1.6.8 + msgpack 1.2.1 + local pip 26.1.2 patched; arxiv 4.0.0 + Crawl4AI 0.9.2 + venv rich sync.)*
