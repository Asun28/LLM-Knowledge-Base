...
Current shipped phases and per-cycle tallies live in `CHANGELOG.md` (brief compact index, newest first; full per-cycle detail in `CHANGELOG-history.md`). Latest full-suite count: 2743 tests across 234 test files; last full run was 2734 passed + 9 skipped (cycle 23 — adds PEP-562 MCP boot-leanness pins + rebuild_indexes CLI regression + hermetic ingest→query→lint E2E + cross-process `file_lock` multiprocessing regression). Open work and deferred-feature roadmap live in `BACKLOG.md`.
