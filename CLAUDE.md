...
- **State:** v0.12.0 · 3288 passed + 24 skipped tests / ~232 files (Windows local; ubuntu-latest CI strict-gated since cycle 36; windows-latest CI matrix deferred to cycle-53+ per cycle-36 L1 CI-cost discipline + cycle-39..69 carry-over). Counts at cycle-69 branch HEAD post-Step-12 CI gate (+14 net tests over cycle-68 across 3 new cycle-69 test files [`tests/test_cycle69_{app_segment_aware_lockin,graph_builder_intentional_bypasses,snapshots}.py`] + 6 C11-L1 inspect.getsource batch upgrades + 4 freeze-and-fold deletions; cycle-69 ships ZERO `src/kb/` changes). Shipped → `CHANGELOG.md` (index) + `CHANGELOG-history.md` (per-cycle detail). Open → `BACKLOG.md`.
...
- **Wiki-context boundary fence** (cycle 70 AC11) — `kb.utils.text.wrap_wiki_context(text)` wraps wiki content with `<wiki_context>...</wiki_context>` tags + system-prompt-style assertion before LLM injection. Mirrors the cycle-7 AC23 `wrap_purpose` pattern. Empty input short-circuits to `""` (T4); literal `</wiki_context>` substrings escaped to `</wiki-context>` (T3). Two in-scope sites: `query/engine.py:1063` (synthesis prompt combined context) + `mcp/core.py:417-432` (Claude Code mode response). Callers reserve `_FENCE_OVERHEAD` from any context budget (T5 — applied at `engine.py:1054`). 4 out-of-scope sibling surfaces (kb_search snippets / kb_read_page body / build_fidelity_context / proposer.py:_relevance_score) filed as Phase 4.5 LOW for cycle-71+.
- **Wiki-context boundary fence** (cycle 70 AC11) — `kb.utils.text.wrap_wiki_context(text)` wraps wiki content with `<wiki_context>...</wiki_context>` tags + system-prompt-style assertion before LLM injection. Mirrors the cycle-7 AC23 `wrap_purpose` pattern. Empty input short-circuits to `""` (T4); literal `</wiki_context>` substrings escaped to `</wiki-context>` (T3). Two in-scope sites: `query/engine.py:1063` (synthesis prompt combined context) + `mcp/core.py:417-432` (Claude Code mode response). Callers reserve `_FENCE_OVERHEAD` from any context budget (T5 — applied at `engine.py:1054`). 4 out-of-scope sibling surfaces (kb_search snippets / kb_read_page body / build_fidelity_context / proposer.py:_relevance_score) filed as Phase 4.5 LOW for cycle-71+.
