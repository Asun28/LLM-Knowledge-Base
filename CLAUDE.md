...
- `rebuild_indexes(wiki_dir=None, *, hash_manifest=None, vector_db=None) -> dict` — In `kb.compile.compiler` (cycle 23 AC2). Operator-scoped "clean-slate" helper that unlinks `HASH_MANIFEST` under `file_lock(..., timeout=1.0)` (prevents racing a concurrent `compile_wiki` save), unlinks the vector DB (`_vec_db_path(wiki_dir)`), clears three LRU cache families (`kb.ingest.extractors.clear_template_cache`, `kb.utils.pages._load_page_frontmatter_cached`, `kb.utils.pages.load_purpose`), and appends one `rebuild-indexes` line to `wiki/log.md` best-effort (audit `OSError` logs warning + sets `audit_written=False`). Returns `{"manifest": {"cleared": bool, "error": str|None}, "vector": {"cleared": bool, "error": str|None}, "caches_cleared": list[str], "audit_written": bool}`. Validates `wiki_dir` resolves under `PROJECT_ROOT` (dual-anchor: the caller-supplied absolute path AND the `.resolve()` target must both land inside, defeating symlink-escape) via shared helper `_validate_path_under_project_root(path, field_name) -> None` (cycle 29 AC2 — void return, raises-only). Cycle-29 AC2 extends the same dual-anchor check to the keyword-only `hash_manifest` + `vector_db` overrides when caller-supplied (`None` defaults skip validation because they derive from `PROJECT_ROOT` by construction). Raises `ValidationError("<field_name> must be inside project root")` on containment failure for any of the three paths. Cycle 29 AC1: audit-log line + `kb rebuild-indexes` CLI stdout render compound `<field>=cleared (warn: <error>)` when `cleared=True AND error` truthy (e.g. sibling `.tmp` unlink failed while main succeeded) via shared `_audit_token(block) -> str` helper. CLI surface `kb rebuild-indexes [--wiki-dir PATH] [--yes]` — no MCP tool (cycle 23 scope; future cycle may add `kb_rebuild_indexes`).
