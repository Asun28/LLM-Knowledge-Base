...
Current shipped phases and per-cycle tallies live in `CHANGELOG.md` (brief compact index, newest first; full per-cycle detail in `CHANGELOG-history.md`). Latest full-suite count: 2801 tests across 244 test files; last full run was 2792 passed + 9 skipped (cycle 27 — adds 4 read-only CLI subcommands for MCP parity: `kb search`, `kb stats`, `kb list-pages`, `kb list-sources`, plus `_format_search_results` helper extraction + `--wiki-dir` containment validation). Open work and deferred-feature roadmap live in `BACKLOG.md`.
