...
- **Phase 3.91 (complete, v0.9.10) — code-review fix list (5-agent parallel review):** 574 tests. **Fixed — Critical/Important:** `save_manifest` now uses `atomic_json_write` (only JSON store not yet atomic). `inject_wikilinks` frontmatter split now uses regex (`_FRONTMATTER_RE = re.compile(r"\A(---\n.*?\n---\n?)(.*)", re.DOTALL)`) — correct for `---` inside YAML values (replaces broken `str.split("---", 2)`). `resolve_wikilinks` and `build_backlinks` now wrap `read_text()` in `try/except (OSError, UnicodeDecodeError)` (asymmetry with `build_graph` fixed). `KNOWN_LIST_FIELDS` extended: `key_arguments`, `quotes`, `themes` (book.yaml), `open_questions` (conversation.yaml). Mermaid `_safe_node_id` now tracks `seen` IDs with suffix deduplication (prevents node collision for `fine-tuning` vs `fine_tuning`). `run_all_checks` with `fix=True` removes fixed dead link issues from report (no longer reports fixed issues as active errors). `kb_ingest` (API mode + extraction mode) wrapped in `try/except`; `path.read_text()` guarded. `kb_create_page` uses `_validate_page_id(check_exists=False)` instead of duplicate traversal check. URL in `kb_ingest_content`/`kb_save_source` YAML header wrapped in `yaml_escape()` + quoted. `kb_refine_page` and `kb_reliability_map` wrapped in `try/except`. `VERDICTS_PATH` and LLM retry constants (`LLM_MAX_RETRIES`, `LLM_RETRY_BASE_DELAY`, `LLM_RETRY_MAX_DELAY`, `LLM_REQUEST_TIMEOUT`) moved to `config.py` (re-exported from `llm.py` for backwards compatibility). `detect_source_drift` bare `except` narrowed to specific types with warning log. `extract_raw_refs` extended to match `.csv`, `.png`, `.jpg`, `.jpeg`, `.svg`, `.gif`. `yaml_escape` logs warning on null byte removal. **Remaining (Phase 3.92):** `review/refiner.py` missing 10k entry cap on review history (feedback + verdicts both cap, review history does not). `kb_read_page`, `kb_list_sources` missing outer `try/except` (raw `OSError`/`PermissionError` can still escape to MCP client). `fix_dead_links` sets `modified=True` and appends fix record even when `re.sub` made no substitution — generates phantom audit trail entries. `inject_wikilinks` `\b` word-boundary regex silently fails for titles with leading non-word chars (`C++`, `GPT-4o`, `.NET`). `compile_wiki` silently drops `pages_skipped`/`wikilinks_injected`/`affected_pages`/`duplicate` from `ingest_source` result. `find_connection_opportunities`/`suggest_new_pages` in evolve have unguarded `read_text()` calls; `evolve/analyzer.py` has no module-level logger; `check_staleness` uses broad `except Exception`. CLAUDE.md and MCP instructions claim 26 tools, actual registered count is 25. `_format_ingest_result` dead dict branch for `affected_pages` (backlink/shared categorization lost — pipeline always returns flat list). `check_source_coverage` reads each page file twice. Hardcoded `0.1` trend threshold in `trends.py` should be `VERDICT_TREND_THRESHOLD` config constant. `wiki_log.py` calls `stat()` twice on same file.
