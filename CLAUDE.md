...
- **State:** v0.12.0 · 3274 passed + 24 skipped tests / ~232 files (Windows local; ubuntu-latest CI strict-gated since cycle 36; windows-latest CI matrix deferred to cycle-53+ per cycle-36 L1 CI-cost discipline + cycle-39..68 carry-over). Counts at cycle-68 branch HEAD post-Step-12 CI gate (+25 new tests across 6 new cycle-68 test files: `tests/test_cycle68_{cli_backend_popen,lint_yaml,audit_docstrings,graph_cache_caller_migrations,httpx_pin_drift,backlog_cleanup_lockin}.py`; 92% cli_backend coverage). Shipped → `CHANGELOG.md` (index) + `CHANGELOG-history.md` (per-cycle detail). Open → `BACKLOG.md`.
