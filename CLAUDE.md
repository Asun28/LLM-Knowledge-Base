...
Key usage:
- **kb** — The knowledge base MCP server (`kb.mcp_server`, 29 tools — cycle 20 added `kb_refine_sweep` + `kb_refine_list_stale`). Start with `kb mcp` or `python -m kb.mcp_server`. Claude Code is the default LLM — no API key needed.
