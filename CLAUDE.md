...
Current shipped phases and per-cycle tallies live in `CHANGELOG.md` (brief compact index, newest first; full per-cycle detail in `CHANGELOG-history.md`). Latest full-suite count: 2767 tests across 239 test files; last full run was 2758 passed + 9 skipped (cycle 24 — adds evidence-trail inline render + sentinel section-span search with fenced-code masking + vector-index atomic rebuild + `file_lock` exponential backoff). Open work and deferred-feature roadmap live in `BACKLOG.md`.
