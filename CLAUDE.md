...
- `ingest_source(path, source_type=None, extraction=None, *, defer_small=False)` — Core ingest function. Accepts optional pre-extracted dict to skip LLM call (used by MCP server in Claude Code mode). Returns `affected_pages` for cascade updates. With `defer_small=True`, sources under `SMALL_SOURCE_THRESHOLD` get summary-only processing. In `kb.ingest.pipeline`.
