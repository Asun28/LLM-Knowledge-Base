...
Current shipped phases and per-cycle tallies live in `CHANGELOG.md` (brief compact index, newest first; full per-cycle detail in `CHANGELOG-history.md`). Latest full-suite count: 2766 tests across 239 test files; last full run was 2757 passed + 9 skipped (cycle 24 — adds evidence-trail inline render + sentinel section-span search + vector-index atomic rebuild + `file_lock` exponential backoff). Open work and deferred-feature roadmap live in `BACKLOG.md`.
