...
574 tests across 38 test files. Core tests: `test_compile.py` (11), `test_evolve.py` (11), `test_graph.py` (8), `test_ingest.py` (8), `test_lint.py` (14), `test_models.py` (4), `test_query.py` (12), `test_cli.py` (5). Phase 2 tests: `test_feedback.py` (14), `test_review.py` (16), `test_lint_semantic.py` (8), `test_mcp_phase2.py` (10). Fix/upgrade tests: `test_fixes_v050.py` (21), `test_fixes_v060.py` (31), `test_utils.py` (33), `test_v070.py` (28), `test_bm25.py` (19), `test_v090.py` (43). v0.9.1 tests: `test_llm.py` (28), `test_lint_verdicts.py` (12), `test_paths.py` (6), `test_mcp_browse_health.py` (15), `test_mcp_core.py` (14), `test_mcp_quality_new.py` (12). v0.9.2 tests: `test_ingest_fixes_v092.py` (8), `test_lint_query_fixes_v092.py` (12), `test_validation_fixes_v092.py` (11). v0.9.3 tests: `test_compiler_mcp_v093.py` (11), `test_lint_fix_v093.py` (6). v0.9.4 tests: `test_tier1_fixes_v094.py` (8), `test_stub_detection_v094.py` (6), `test_drift_detection_v094.py` (7). v0.9.5 tests: `test_tier1_audit_v095.py` (13). v0.9.6 tests: `test_tier2_audit_v096.py` (18). v0.9.7 tests: `test_tier3_fixes_v097.py` (7). v0.9.8 tests: `test_v098_fixes.py` (29). v0.9.9 tests: `test_v099_phase39.py` (33). v0.9.9 review fixes: `test_review_fixes_v099b.py` (21).
...
- **Phase 3.9 (complete, v0.9.9):** Infrastructure for content growth and AI leverage — 8 features implemented, 32 new tests (518→550), 2 new MCP tools (23→25). Environment-configurable model tiers (`CLAUDE_SCAN_MODEL`, `CLAUDE_WRITE_MODEL`, `CLAUDE_ORCHESTRATE_MODEL` env vars override defaults in `config.py`). PageRank-blended search ranking (`final_score = bm25_score * (1 + PAGERANK_SEARCH_WEIGHT * normalized_pagerank)` — well-linked pages rank higher, `PAGERANK_SEARCH_WEIGHT=0.5` config). Duplicate detection in ingest (hash-based dedup in `ingest_source()` — checks compile manifest for existing sources with identical content hash, verifies other source still exists). Verdict trend dashboard (`kb_verdict_trends` MCP tool — weekly pass/fail/warning rates, quality trend direction, new `kb.lint.trends` module). Mermaid graph export (`kb_graph_viz` MCP tool — exports knowledge graph as Mermaid flowchart with auto-pruning to top N nodes by degree, subgraph grouping by page type, new `kb.graph.export` module). Retroactive inbound wikilink injection (`inject_wikilinks()` in `kb.compile.linker` — scans existing pages for plain-text mentions of new page titles, injects `[[wikilinks]]` using word-boundary matching, case-insensitive, skips frontmatter and existing links — NOTE: utility implemented but integration into `ingest_source()` completed in review fix below). Content-length-aware ingest tiering (`SMALL_SOURCE_THRESHOLD=1000` — sources under threshold get summary-only processing when `defer_small=True`, prevents stub proliferation). Cascade update detection on ingest (`ingest_source()` returns `affected_pages` — backlinks + shared-source pages, makes `kb_affected_pages` actionable at ingest time — NOTE: MCP output surfacing completed in review fix below).
- **Phase 3.9 review fixes (v0.9.9, post-code-review):** 4 issues fixed from code review, 14 new tests (550→564). `inject_wikilinks` integrated into `ingest_source()` — automatically called for each newly created page (lazy import avoids circular dependency), result includes `wikilinks_injected` list. `_format_ingest_result` shows "Duplicate content detected" message with hash when `duplicate=True` instead of silent "0 pages created". `_format_ingest_result` surfaces `affected_pages` (backlinks + shared-source pages) in `kb_ingest` output — actionable cascade review list. `ingest_source` docstring updated with all new parameters (`defer_small`) and return keys (`pages_skipped`, `affected_pages`, `wikilinks_injected`, `duplicate`).
- **Phase 3.9 DRY review fixes (v0.9.9, post-code-review round 2):** 3 new tests (564→567). Bugfix: `_update_existing_page` now accepts `verb: str = "Mentioned"` parameter — concept pages updated via a second source now correctly write `"Discussed in"` instead of always `"Mentioned in"` (pre-existing bug surfaced by DRY refactor). `_process_item_batch` derives `subdir` from `_SUBDIR_MAP[page_type]` internally (removes redundant caller-supplied `subdir` arg, eliminates mismatch risk). `_update_index_batch` uses `_SUBDIR_MAP.get(page_type)` with a null guard (prevents future `KeyError` if a page type is added to `_SECTION_HEADERS` without a matching entry). `valid_items` docstring clarified (covers both created and updated pages, not only new ones). Lazy imports restored for `build_backlinks`/`inject_wikilinks` (top-level import introduced circular import risk via `kb.compile.__init__` → `kb.compile.compiler` → `kb.ingest.pipeline`). Test additions: `test_bad_timestamp_still_counted_in_overall` (verdicts with invalid timestamps count in `overall` but not `period_buckets`), `test_backoff_delay_values` + `test_backoff_delay_cap` (`_backoff_delay` independently verified including cap behavior).
- **Phase 3.9 MCP output fixes (v0.9.9, post-code-review round 3):** 7 new tests (567→574). Two `_format_ingest_result` bugs fixed: (1) `affected_pages` flat `list[str]` from `_find_affected_pages` was silently dropped by `isinstance(affected, dict)` guard — now handled with `elif isinstance(affected, list)` branch so cascade review hints appear in `kb_ingest` output; (2) `wikilinks_injected` key existed in ingest result and was documented but never read by the formatter — now shown as "Wikilinks injected (N):" section. `TestAffectedPagesFlatList` (3 tests) and `TestWikilinksInjectedMCPOutput` (4 tests) added to `test_review_fixes_v099b.py`. Verified with Context7 that `tool_choice: {"type": "tool", "name": ...}` forced tool use pattern is correct for current Anthropic SDK (0.89.0); `messages.parse()` with Pydantic noted as future simplification option.
- **Phase 3.91 (complete, v0.9.10) — code-review fix list (5-agent parallel review):** Critical/important fixes. `save_manifest` now uses `atomic_json_write` (only JSON store not yet atomic). `inject_wikilinks` frontmatter split now uses regex (`_FRONTMATTER_RE = re.compile(r"\A(---\n.*?\n---\n?)(.*)", re.DOTALL)`) — correct for `---` inside YAML values (replaces broken `str.split("---", 2)`). `resolve_wikilinks` and `build_backlinks` now wrap `read_text()` in `try/except (OSError, UnicodeDecodeError)` (asymmetry with `build_graph` fixed). `KNOWN_LIST_FIELDS` extended: `key_arguments`, `quotes`, `themes` (book.yaml), `open_questions` (conversation.yaml). Mermaid `_safe_node_id` now tracks `seen` IDs with suffix deduplication (prevents node collision for e.g. `fine-tuning` vs `fine_tuning`). `run_all_checks` with `fix=True` removes fixed dead link issues from report (no longer reports fixed issues as active errors). `kb_ingest` (API mode + extraction mode) wrapped in `try/except`; `path.read_text()` guarded. `kb_create_page` uses `_validate_page_id(check_exists=False)` instead of duplicate traversal check. URL in `kb_ingest_content`/`kb_save_source` YAML header wrapped in `yaml_escape()` + quoted. `kb_refine_page` and `kb_reliability_map` wrapped in `try/except`. `VERDICTS_PATH` and LLM retry constants (`LLM_MAX_RETRIES`, `LLM_RETRY_BASE_DELAY`, `LLM_RETRY_MAX_DELAY`, `LLM_REQUEST_TIMEOUT`) moved to `config.py` (re-exported from `llm.py` for backwards compatibility). `detect_source_drift` bare `except` narrowed to specific types with warning log. `extract_raw_refs` extended to match `.csv`, `.png`, `.jpg`, `.jpeg`, `.svg`, `.gif`. `yaml_escape` logs warning on null byte removal.
- **Phase 3.91 prior (planned) — original note:** Fixes from 4-agent parallel review of v0.9.9 codebase. **Critical:** 7 failing tests — duplicate detection tests (`TestDuplicateDetection` ×3, `TestContentLengthIngestTiering` ×2, `TestCascadeUpdateOnIngest` ×2) fail because tests patch `HASH_MANIFEST` constant instead of mocking `load_manifest` directly; `test_llm.py` imports `RETRY_BASE_DELAY`/`RETRY_MAX_DELAY` from wrong module (`kb.config` instead of `kb.utils.llm`). **Data integrity:** `save_manifest` not atomic — use `atomic_json_write` (only JSON store not yet converted); `review/refiner.py` missing 10k entry cap on review history (feedback and verdicts both cap, review history does not). **MCP error handling:** `kb_refine_page`, `kb_reliability_map`, `kb_read_page`, `kb_list_sources` missing outer `try/except` — raw `OSError`/`PermissionError` can escape to MCP client, violating the "all tools return `Error:...` strings" contract. **Correctness:** `fix_dead_links` sets `modified=True` and appends fix record even when `re.sub` made no substitution — generates phantom audit trail entries; `inject_wikilinks` word-boundary `\b` regex silently fails for titles with non-word chars (`C++`, `GPT-4o`, `.NET`); `compile_wiki` silently drops `pages_skipped`/`wikilinks_injected`/`affected_pages`/`duplicate` from `ingest_source` result — plan deviation, these must be surfaced. **Error handling gaps:** `resolve_wikilinks` and `find_connection_opportunities`/`suggest_new_pages` in evolve have unguarded `read_text()` calls (inconsistent with `inject_wikilinks` in same file); `detect_source_drift` uses bare `except Exception: continue` with no logging; `evolve/analyzer.py` has no module-level logger; `check_staleness` uses broad `except Exception` instead of specific types; flagged_pages failure silently suppressed with no log. **Docs:** CLAUDE.md and MCP instructions claim 26 tools, actual registered count is 25 — fix count. **Minor:** `_format_ingest_result` dead dict branch for `affected_pages` (pipeline always returns flat list, backlink/shared categorization lost); `check_source_coverage` reads each page file twice; hardcoded `0.1` trend threshold in `trends.py` should be a `VERDICT_TREND_THRESHOLD` config constant; `wiki_log.py` calls `stat()` twice on same file.
