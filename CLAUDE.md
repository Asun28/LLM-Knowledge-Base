...
- **Tier-boundary verifier** (cycle 73 AC03+AC04) — `kb.lint.augment.orchestrator._validate_tier_boundary(scan_output, *, expected_keys, max_depth=4, max_string_len=4096) -> dict` re-gates scan-tier `_call_llm_json` outputs at the L394 site BEFORE orchestrate-tier consumption (rejects: non-dict root, extra keys not in expected_keys, strings > 4096 chars, nesting > depth 4, unsupported value types). Anti-spoofing (T5): `expected_keys` MUST be derived from local schema via `frozenset(schema['properties'].keys())` — NEVER from `scan_output.keys()`. `kb.errors.TierBoundaryError(ValidationError)` exception subclasses `ValidationError` so legacy `except ValidationError` still catches; orchestrator split-catch records distinct manifest reason `"tier_boundary_rejected: ..."` for forensic distinctness. Closes cycle-72 §T8 EscalationOfPrivilege blast-radius gap. Cycle-74+ defers: `max_keys=500` DoS bound (R2 F-2), `proposer.py:91/168` peer call sites (R1 C2), required-keys enforcement (R2 F-3).
