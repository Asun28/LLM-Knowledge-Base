...
- **State:** v0.10.0 · 2826 tests / 247 files (2816 passed + 10 skipped). Shipped → `CHANGELOG.md` (index) + `CHANGELOG-history.md` (per-cycle detail). Open → `BACKLOG.md`.
...
- **Latest full-suite:** 2826 tests / 247 files · 2816 passed + 10 skipped.
- **Latest cycle (29):** `rebuild_indexes` hardening — compound audit-token for partial clears + CLI mirror, dual-anchor `PROJECT_ROOT` containment on override kwargs, doc-hygiene cleanups.
...
Full suite: 2826 tests / 247 files (2816 passed + 10 skipped). New tests per cycle go in versioned files (e.g. `test_cycle20_errors_taxonomy.py`). Per-cycle test-file details → `CHANGELOG-history.md`.
