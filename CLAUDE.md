...
Current shipped phases and per-cycle tallies live in `CHANGELOG.md` (brief compact index, newest first; full per-cycle detail in `CHANGELOG-history.md`). Latest full-suite count: 2809 tests across 245 test files; last full run was 2800 passed + 9 skipped (cycle 28 — closes HIGH-Deferred (b) first-query observability: `VectorIndex._ensure_conn` sqlite-vec extension-load latency + `BM25Index.__init__` corpus-indexing latency, both instrumented with `time.perf_counter` + INFO log + module-level counters + `get_sqlite_vec_load_count()` / `get_bm25_build_count()` getters; + BACKLOG hygiene and CHANGELOG commit-count rule codification). Open work and deferred-feature roadmap live in `BACKLOG.md`.
