...
# Run once at module load so the RAISE-on-unparseable policy does not break
# any session that inherited lock files from pre-cycle-2 runs.
try:
    _purge_legacy_locks()
except Exception as exc:  # pragma: no cover — module-load must never abort
    logger.warning("_purge_legacy_locks failed at import time: %s", exc)
