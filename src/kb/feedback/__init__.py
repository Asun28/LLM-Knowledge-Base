"""Feedback storage + reliability scoring (implementation-only namespace).

Submodules in this package are imported via fully-qualified paths
(``kb.feedback.store``, ``kb.feedback.reliability``); nothing is re-exported
at the package level. ``__all__`` is intentionally empty so star-imports are
