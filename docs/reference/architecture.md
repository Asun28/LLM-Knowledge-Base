...
- **Vector index** (`kb.query.embeddings`): `rebuild_vector_index` is tmp-then-`os.replace` with `_conn` closed before swap (Windows handle-release); stale `.tmp` unlinked on entry. `VectorIndex.build(*, db_path=None)` — override does NOT mutate `self._stored_dim`. Batch callers pass `_skip_vector_rebuild=True` and invoke once at tail. `maybe_warm_load_vector_model(wiki_dir) -> Thread|None` spawns a daemon preload (idempotent no-op if model already loaded); called by `kb.mcp.__init__.main()` after tool registration — `kb compile` intentionally NOT warm-loaded. Cycle 64 adds dim-mismatch auto-rebuild: `VectorIndex.query()` still returns `[]` for the first stored-dim/query-dim mismatch, but validates `_derive_wiki_dir()` with `_validate_path_under_project_root(..., "vector_auto_rebuild_target")`, calls `rebuild_vector_index`, and exposes `get_dim_mismatch_auto_rebuild_count()`. `KB_DISABLE_VECTOR_AUTO_REBUILD=1` is read at call time.
- **Graph cache** (`kb.graph.cache`): cycle 64 canonicalizes lint-pass graph lookup through `get_graph(wiki_dir, *, pages=None)`. Disk-scan callers are cached by `(wiki_dir.resolve().as_posix(), max_mtime_of_wiki_subdirs)` with FIFO eviction at 4 entries. Callers supplying `pages=` bypass the cache so in-memory snapshots cannot poison disk-backed graph calls. Mutators invalidate after successful writes: `ingest_source`, `refine_page`, `compile_wiki`, and lint's `fix=True` dead-link rewrite path. `get_cache_stats()` returns integer counters only, never resolved path keys.
- **Compile auto-publish** (`kb.compile.publish`): cycle 64 has `compile_wiki` invalidate the graph cache, then call `auto_publish_after_compile` unless `KB_DISABLE_COMPILE_AUTO_PUBLISH=1` is set at call time. Artifacts default to `<wiki_dir>.parent/_publish`, outside the wiki walk surface. `auto_publish_after_compile` validates `out_dir` with `_validate_path_under_project_root(..., "publish_out_dir")`, runs the Tier-1 builders plus per-page siblings and sitemap, logs per-builder warnings, and does not fail compile on publish errors.
...
- **Publish siblings manifest** (`kb.compile.publish`): cycle 64 stores `<wiki_dir>.parent/.data/publish-siblings-manifest.json` for per-page sibling outputs. Each run records kept page IDs; the next run unlinks only sibling files for page IDs that were previously emitted but are no longer kept. Missing or corrupt manifest data falls back to the cycle-16 unconditional cleanup behavior for currently-excluded pages.
