...
- **kb** — The knowledge base MCP server (`kb.mcp_server`, 28 tools). Start with `kb mcp` or `python -m kb.mcp_server`. Claude Code is the default LLM — no API key needed.
  ...
  - **Concurrency model** (cycle 95) — FastMCP runs sync tools on anyio's shared default worker pool (40 threads), and a tool holds its thread for its whole runtime. The six LONG tools — `kb_query`, `kb_lint`, `kb_compile`, `kb_ingest`, `kb_ingest_content`, `kb_capture` — are therefore registered as async wrappers by `kb.mcp._offload.register_long_tool` and offloaded to a DEDICATED per-event-loop `anyio.CapacityLimiter` (default 8 threads, `KB_MCP_LONG_TOOL_THREADS`), so a batch of concurrent compiles cannot queue sub-second reads like `kb_search` behind it. Kill-switch `KB_DISABLE_MCP_LONG_TOOL_LIMITER=1` routes them back through the shared pool. The same model is summarised in the server's own `instructions` block under `## Concurrency`. The module attributes stay plain sync callables — importing `kb.mcp.health.kb_lint` and calling it directly is unaffected.
