...
**Cycle 29 R1 Sonnet S2 correction (2026-04-24 post-R1-fix):** actual full-suite count after R1 fixes is **2826** (2809 + 17 new tests), not 2823. The +3 drift from the Q15 projection comes from (a) `TestOverrideEmptyInput` split into its own class (now superseded by `TestOverrideCwdSemantics` with 2 tests post-R1 S1 fix — adds 1 vs plan); (b) Q8's hybrid symlink strategy implemented as two distinct tests (`test_hash_manifest_override_resolve_escape_raises` via `_ResolvingPath` subclass + `test_hash_manifest_override_symlink_to_outside_raises` via `os.symlink` with `@pytest.mark.skipif(os.name == "nt")` decorator per R1 Codex M2), not one; (c) R1 Sonnet S1 fix added 1 new regression test (`test_dot_path_as_wiki_dir_accepted_when_cwd_inside_project`) pinning the cross-platform `Path("") == Path(".")` equivalence. CHANGELOG/CHANGELOG-history/CLAUDE.md carry the authoritative 2826 number.
