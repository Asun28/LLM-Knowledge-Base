...
# -- Cycle 59 fold: query --format CLI flag -------------------------------
# Folded from tests/test_v4_11_cli.py and deleted in the same change.


@pytest.fixture
def cycle59_mocked_query_wiki():
    with patch("kb.query.engine.query_wiki") as m:
        yield m


def test_cycle59_cli_query_default_format_text(cycle59_mocked_query_wiki):
    cycle59_mocked_query_wiki.return_value = {
        "answer": "RAG is...",
        "citations": [],
    ...
    assert result.exit_code == 0
    _, kwargs = cycle59_mocked_query_wiki.call_args
    assert kwargs.get("output_format") in (None, "text")


def test_cycle59_cli_query_markdown_format(cycle59_mocked_query_wiki):
    cycle59_mocked_query_wiki.return_value = {
        "answer": "RAG is...",
        "citations": [],
        "source_pages": [],
...
def test_cycle59_cli_query_rejects_invalid_format():
    runner = CliRunner()
    result = runner.invoke(cli, ["query", "q", "--format", "pdf"])
    assert result.exit_code == 2  # Click usage error


def test_cycle59_cli_query_all_formats_accepted(cycle59_mocked_query_wiki):
    cycle59_mocked_query_wiki.return_value = {
        "answer": "x",
        "citations": [],
        "source_pages": [],
...
def test_cycle59_cli_query_surfaces_output_error(cycle59_mocked_query_wiki):
    cycle59_mocked_query_wiki.return_value = {
        "answer": "x",
        "citations": [],
        "source_pages": [],
