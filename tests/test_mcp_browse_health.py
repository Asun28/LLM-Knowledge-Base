...
# === Cycle 54 — folded from tests/test_cycle8_health_wiki_dir.py ===
# Cycle 8 MCP wiki_dir plumbing coverage (kb_stats / kb_verdict_trends success
# + traversal rejection on TWO different inputs each — richer than the
# parametrized terminator above which asserts only the error prefix).


def _write_health_fold_page(wiki_dir: Path, page_id: str, title: str, body: str = "") -> None:
...
def test_kb_stats_scopes_to_supplied_wiki_dir(tmp_path, monkeypatch):
    ...
    _write_health_fold_page(wiki_dir, "concepts/alpha", "Alpha")
    _write_health_fold_page(wiki_dir, "concepts/beta", "Beta", "[[concepts/alpha]]")
