...
# -- Cycle 59 fold: kb_query MCP output_format ----------------------------
# Folded from tests/test_v4_11_mcp.py and deleted in the same change.


@pytest.fixture
def cycle59_mocked_mcp_query_wiki():
    # Cycle 19 AC15 — patch owner module.
    with patch("kb.query.engine.query_wiki") as m:
        yield m


def test_cycle59_mcp_kb_query_format_requires_use_api(cycle59_mocked_mcp_query_wiki):
...
def test_cycle59_mcp_kb_query_invalid_format():
    from kb.mcp.core import kb_query
...
def test_cycle59_mcp_kb_query_empty_format_default_mode_not_errored(monkeypatch, tmp_wiki):
...
def test_cycle59_mcp_kb_query_format_use_api_success(cycle59_mocked_mcp_query_wiki):
    from kb.mcp.core import kb_query

    cycle59_mocked_mcp_query_wiki.return_value = {
        "answer": "RAG is ...",
...
def test_cycle59_mcp_kb_query_format_case_normalization(cycle59_mocked_mcp_query_wiki):
    from kb.mcp.core import kb_query
...
def test_cycle59_mcp_kb_query_format_text_equals_no_format(cycle59_mocked_mcp_query_wiki):
...
def test_cycle59_mcp_kb_query_format_output_error_surfaced(cycle59_mocked_mcp_query_wiki):
    from kb.mcp.core import kb_query
