...
# ---------------------------------------------------------------------------
# Cycle 39 fold (was tests/test_cycle38_mock_scan_llm_reload_safe.py) —
# pin the cycle-38 AC1 dual-site mock_scan_llm fixture contract per
# cycle-4 L4 freeze-and-fold rule. The kb.capture module is loaded into
# sys.modules at line 20's `from kb.capture import (...)`, satisfying the
# original cycle-38 file's defensive pre-import requirement (security-guard
# at src/kb/capture.py:840 fires on first import; first import here is
# under real PROJECT_ROOT, not a fixture-patched tmp).
# ---------------------------------------------------------------------------
...
class TestMockScanLlmReloadSafety:
    ...
    case (b) in isolation (path wrapped to satisfy ruff E501 line-length):
        ...
        pytest tests/test_capture.py::TestMockScanLlmReloadSafety \\
...
# ── Cycle 10 capture regressions (folded from test_cycle10_capture.py) ───────
...
def test_extract_items_via_llm_rejects_oversize_prompt(monkeypatch):
    ...
    Folded from tests/test_cycle9_capture_runtime_guard.py (cycle 49 — Phase 4.5 HIGH #4).

    Design condition for AC21 specified both (a) module-level
    ``assert CAPTURE_MAX_BYTES <= MAX_PROMPT_CHARS`` AND (b) a runtime check
    inside ``_extract_items_via_llm``'s body so a legitimately-sized content
    combined with an oversize template still raises before the LLM call. The
    module-level ``assert`` vanishes under ``python -O``; this test exercises
    the runtime guard.
    """
...
# Folded from `tests/test_cycle17_capture_two_pass.py` — cycle 17 AC10:
# capture two-pass write with all-or-nothing rollback.
# Helper renamed: _make_items → _make_two_pass_items per C52-L4.


def _make_two_pass_items(n: int) -> list[dict]:
    return [
        {
            "kind": "discovery",
            "title": f"item {i}",
