...
- `ingest/pipeline.py` state-store fan-out — a single `ingest_source` mutates summary page, N entity pages, N concept pages, `index.md`, `_sources.md`, `.data/hashes.json`, `wiki/log.md`, `wiki/contradictions.md`, plus N `inject_wikilinks` writes. Every step is independently atomic, **none reversible**. (R2)
  **Cycle 83 closed the data-loss half (Design C).** The manifest hash is now committed exactly once, as the last durable step of a successful ingest (`_commit_ingest_manifest`, after `_run_ingest_body` returns), so a crash before it leaves no manifest entry and `find_changed_sources` re-selects the source. Same-process concurrent ingest of identical content is serialized by an in-process per-content-hash lock. Manifest values are uniformly bare hashes (the `in_progress:`/`failed:` marker vocabulary is retired from the ingest path). Note the original entry text understated the window: the reservation happened BEFORE the first page write, not between manifest-write and log-append, so the exposure spanned the whole ingest body. Two rejected approaches are recorded in the design doc: the receipt-file remedy proposed here (records the crash but leaves `manifest[K] = H` intact, so the skip is not fixed) and an `in_progress:` marker (Codex found it reintroduced the data loss via mutual marker suppression).
  Still open: no rollback of the wiki-side writes themselves. A crashed ingest is now correctly re-ingestable, but the partial pages it wrote stay on disk until the retry overwrites them. Deferred deliberately — page writes are irreversible and may carry human edits, so unwinding them needs its own design (see the `compile_wiki` per-source rollback MEDIUM item, same fix family).
...
- `compile/compiler.py` `in_progress:` markers are immortal (cycle-83 threat T6) — marker-valued entries are exempt from BOTH prune sites so cycle-25 AC7's "operator decides" contract holds. Only `compile_wiki`'s cycle-25 premarker still produces these (cycle 83's Design C retired markers from the `ingest_source` path), so the producer set is narrow, but a `compile_wiki` hard-killed between premarker and ingest-commit leaves an immortal `in_progress:` entry, and AC7's warning enumerates every stale key by name — a repo with many gets an unbounded log line. (fix: add an age field or a sidecar `.data/marker_ages.json`, cap the enumerated list at N with an "and M more" suffix, and offer an explicit `kb compile --clear-stale-markers` rather than auto-deletion, which cycle-25 Q10 rejected for racing a live compile.)

- `tests/` contradiction-concurrency tests are load-sensitive (cycle-83 discovery) — `tests/test_ingest.py::test_persist_contradictions_concurrent` and `tests/test_cycle8_contradictions_idempotent.py::test_concurrent_same_day_same_source_distinct_claims_both_persist` both spawn two threads that contend on `file_lock(contradictions.md)` and assert both payloads land. They pass reliably in isolation but were observed to fail intermittently under heavy full-suite load (and were separately confounded during cycle 83 by orphan `in_progress:` markers a different test wrote into the REAL `.data/hashes.json`; that pollution is cleaned and cycle-83's Design C no longer writes markers, so this specific trigger is gone). The underlying timing fragility remains — the tests rely on wall-clock thread overlap rather than a deterministic barrier. (fix: give both tests a `threading.Event` each thread sets once it holds the lock so the other's attempt fully resolves before assertions run — mirrors the cycle-82b MINOR-3 fix that made the `page_lock` exclusion tests event-driven. Alternatively raise `LOCK_TIMEOUT_SECONDS` for these two tests only.)

- `ingest/pipeline.py` duplicate sources are re-extracted on every compile (cycle-83 R2/R3) — a source whose content duplicates an already-ingested source owns no wiki pages, so cycle 83 (Design C) does NOT record it in the manifest (recording its bare hash would make it a false dedup target — the R3 self-propagating data-loss regression). Consequence: `find_changed_sources` classifies the duplicate as "new" every compile, so `ingest_source` runs a full `extract_from_source` (an LLM call) before the duplicate check catches it, each time. Wasteful for repos that keep identical content in two raw files; not data loss. Closing it needs the manifest (or a sidecar) to record page OWNERSHIP — "source X owns the pages for content H" vs "source Y is a pointer to X" — so a duplicate can be marked processed-and-skippable without becoming a dedup target. Deferred deliberately: the ownership model is a real design surface and every marker-in-the-value shortcut tried in cycle 83 reintroduced a data-loss ambiguity. (fix: a `dup_of` sidecar map, or a manifest value shape that encodes ownership, plus `find_changed_sources` exemption for pointer entries AND exclusion of pointer entries from the dedup-target scan.)

- `utils/io.py` Windows has NO rename-durability barrier (cycle-86 Codex review MAJOR) — `_fsync_parent_dir` returns immediately when `os.name == "nt"`, because NTFS has no `O_DIRECTORY` and `os.open` on a directory raises there. The docstring claims `MoveFileEx` covers it, but `os.replace` does not request `MOVEFILE_WRITE_THROUGH`, so it makes no power-loss durability promise. Windows is this project's PRIMARY development platform, so cycle-86 AC04 currently hardens only the CI platform. Not a regression (pre-cycle-86 there was no barrier anywhere) but AC04's goal is unmet where it matters most. (fix: after `os.replace`, re-open the DESTINATION file and `os.fsync` its handle — on NTFS that flushes the metadata that carries the rename; or call `MoveFileExW` with `MOVEFILE_WRITE_THROUGH` via `ctypes`. Needs a real power-loss reasoning pass and a Windows-specific test, so it is its own cycle.)

- `capture.py` `query/embeddings.py` same-class rename peers lack any durability barrier (cycle-86 Codex review MINOR) — `capture.py:694` returns a successful capture after a bare `os.replace` with neither file nor parent-directory fsync; `query/embeddings.py:363` returns a successful vector rebuild the same way. Cycle 86 AC04 hardened `atomic_json_write` + `_atomic_text_write_replace`; these two bypass both helpers and call `os.replace` directly. Power loss can leave a reported capture missing or partial, or silently restore the previous vector DB while the caller reports success. (fix: route both through `atomic_text_write` / `atomic_json_write`, or call `_fsync_parent_dir` explicitly after their renames. Prefer the former — a third bespoke write path is how this class keeps reappearing.)

- `lint/checks/evidence_resolvable.py` containment TOCTOU (cycle-86 Codex review MINOR) — containment is checked against the resolved path, then `.is_file()` runs. Between the two, replacing an ancestor beneath `raw/` with a symlink/junction to an outside directory makes the stat follow it and touch a path outside `raw/`. Requires local write access to `raw/`, which is a much weaker attacker position than the frontmatter-only vector cycle 86 closed, and the leak is a boolean rather than content. Inherent to check-then-use on a mutable filesystem. (fix: `os.open(..., O_NOFOLLOW)` + `fstat` on the descriptor, mirroring the cycle-65 AC10 `_open_no_follow` mitigation in `rebuild_indexes`, so the containment decision and the stat refer to the same inode.)
...
- `lint/augment/tier_boundary.py` `capture.py` NESTED value-domain enforcement — cycle 86 AC02 shipped `allowed_values` for ROOT-LEVEL keys only, which covers `proposer.py`'s `action` enum but NOT `capture.py:280,286`'s `kind` / `confidence` enums, which live inside `items[].properties` of `_CAPTURE_SCHEMA`. Those two remain enforced solely by `_call_llm_json`'s jsonschema pass (`confidence` gets a second check downstream via `validate_frontmatter`; `kind` gets none — `capture.py:522` consumes `item["confidence"]` with no membership re-check). Deliberate cycle-86 scope-out per design Q3: reaching nested keys means inventing a path mini-language (`items[].kind`) inside a security validator, which is new parsing surface in exactly the wrong place. Revisit only if a real out-of-vocabulary `kind` is observed, or if a third nested-enum schema appears — two call sites do not yet justify the abstraction.
  (effort: Medium — nested-path syntax or a per-schema walker + anti-spoofing rule extended to nested derivation; do NOT accept a caller-supplied nested map that could be built from model output)
...
- Multi-signal graph retrieval — BM25 seed → 4-signal graph expansion (direct ×3 + source-overlap ×4 + Adamic-Adar ×1.5 + type-affinity ×1). Prerequisite: typed semantic relations. Source: nashsu. **Check before starting:** the bounded-traversal substrate is tracked separately under §"HIGH LEVERAGE — nashsu/llm_wiki + TencentDB-Agent-Memory review (2026-07-25)" above. If that ships first, this entry reduces to a scoring change over an existing traversal — don't rebuild the walk.
