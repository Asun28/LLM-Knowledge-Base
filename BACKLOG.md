...
- `tests/` Windows reparse-point coverage for the evidence-stat no-follow check (cycle-87 R1 Codex MINOR-5) — `_is_regular_file_no_follow` is pinned on POSIX by `test_stat_does_not_follow_a_symlink_swapped_in_after_the_containment_check`, and the platform-independent `test_existence_verdict_is_driven_by_the_no_follow_helper` detects a revert to `Path.is_file()`. What is NOT covered is Windows-specific reparse behaviour: file symlink, directory junction, and hardlink discriminators. Deferred rather than stubbed because unprivileged symlink creation fails on the dev machine (`WinError 1314`), so the test would skip on the only platform that could run it; junctions (`mklink /J`) do not need privileges and are the viable starting point. R1 explicitly agreed the existing tests should be RETAINED as non-regression guards, so this is added coverage, not a correction. (fix: a Windows-only test creating a junction under `raw/` via `mklink /J`, asserting the stat refuses it; plus a hardlink case asserting hardlinks are still ACCEPTED, since `S_ISREG` is true for them.)

- `capture.py` all-or-nothing rollback is best-effort and does not report indeterminacy (cycle-87 R2 Codex MAJOR-1 residual) — cycle 87 closed the reported defect (a completed promote whose barrier failed left an orphan `<slug>.md` after a capture reported `([], error)`; `RenameCompletedBarrierError` now drives an unlink). What remains is the ROLLBACK's own reliability: `_rollback_finalized` and the new unlink both swallow `OSError` with a warning, and no `fsync` of the captures directory follows the deletions, so on a mid-rollback failure or power loss items 1..N-1 can survive or reappear while the caller was told nothing was written. Pre-existing in shape — best-effort rollback predates this cycle — but the barrier-failure path makes it reachable more often. (fix: check every rollback unlink, fsync the captures dir once after all deletions, and return a distinct indeterminate-rollback error so the caller knows the batch state is unknown rather than empty; test an item-3-of-5 barrier failure.)

- `review/refiner.py` marks a revision `failed` when the page write succeeded but was not durable (cycle-87 R2 Codex MAJOR-2) — `refine_page` wraps `atomic_text_write` in a broad catch that records the revision as `failed` and returns an error. When only the durability barrier fails, `page_path` already contains `new_text`, so the revision state contradicts the file. NOT introduced by cycle 87: cycle 86 already made `_fsync_parent_dir` raise on `EIO`/`ENOSPC` on POSIX, so this path existed then; cycle 87 makes it reachable on Windows too and gives it a distinct type. Not data loss and a retry is idempotent, so it is a reporting-accuracy defect rather than a correctness one. (fix: catch `RenameCompletedBarrierError` separately and either retry the barrier or record an applied-but-not-durable state, which is a change to the revision state machine and wants its own design pass.)
