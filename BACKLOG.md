...
- `lint/augment/tier_boundary.py` `lint/augment/proposer.py` closed action vocabulary for LLM-proposed operations — the permitted-action enum exists today only as JSON-schema text plus a per-call-site `if` fallback (`proposer.py:46` declares `enum: ["propose", "abstain"]`; `proposer.py:120-123` re-checks it by hand after the boundary), so `_validate_tier_boundary` itself passes an out-of-vocabulary action straight through and every new call site has to re-implement the check. Centralize it: add a value-domain parameter to the validator and reject out-of-vocabulary values at the tier boundary before orchestrate-tier consumption, recording a distinct manifest reason (`action_not_in_vocabulary: ...`) for forensic distinctness from the existing `tier_boundary_rejected: ...`. Not to be confused with `review/refiner.py`'s `_SWEEP_ALLOWED_ACTIONS` (`refiner.py:365`) — that is a caller-supplied `mark_failed`/`delete` vocabulary for the stale-pending sweep, already validated by membership, with no LLM in the path. nashsu constrains its async review queue to exactly `create page` / `deep research` / `skip`, with the rationale stated as "约束操作防止 LLM 凭空生成任意操作" — constrain the operation set so the model cannot invent arbitrary actions. Direct same-class peer of the cycle-73/74 `_validate_tier_boundary` work: adds a value-domain check alongside the existing `expected_keys` / `required_keys` key-domain checks. Anti-spoofing rule (T5) carries over unchanged — the enum MUST be derived from the local schema, NEVER from the model output. Source: nashsu/llm_wiki README_CN §12 (审核系统).
