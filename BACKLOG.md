...
- `utils/pages.py` `load_page_frontmatter` unbounded page-body read (cycle-96 R1 F1) — cycle 96 bounded the SOURCE reads behind `kb_review_page` / `kb_lint_deep` (`PAIRED_SOURCE_READ_MAX_BYTES`) but the wiki page body itself is still read whole, via `load_page_frontmatter(page_path)` → `frontmatter.loads`. A 200 MB page body is loaded in full before `_cap_page_content` trims it for assembly — the same read-vs-output asymmetry cycle 96 closed on the other half. Deliberately NOT fixed in cycle 96: `load_page_frontmatter` is the shared frontmatter reader for the whole lint/query hot path (`scan_wiki_pages`, every `lint/checks/*`, `graph/builder`, `query/engine`) behind an `lru_cache(maxsize=8192)`, so a byte cap there changes what EVERY caller sees, and a partial read cannot be cached under the same key as a full one without a correctness story. Lower severity than the source half: wiki pages are LLM-written and system-owned, whereas `raw/` holds arbitrary user-supplied scrapes, and the ingest-side rewriter cap (`J1`) already bounds what gets written.
  (fix: either a size check in `load_page_frontmatter` that raises/flags above a cap — with the cache key extended to record whether the entry is partial — or a separate bounded reader used only by the two paired-context builders, leaving the shared hot path untouched. The second is smaller and matches the cycle-96 precedent.)
