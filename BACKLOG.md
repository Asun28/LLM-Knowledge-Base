...
- `requirements.txt` split into per-extra files (cycle-37 follow-up — deferred from cycle 36 per design Q7=B) — cycle 34 AC2 declared `[project.optional-dependencies]` extras (`hybrid`/`augment`/`formats`/`eval`/`dev`) in `pyproject.toml`, but `requirements.txt` remains a full-dev superset. Cycle 37 ships `requirements-runtime.txt` + 5 per-extra files (`requirements-{hybrid,augment,formats,eval,dev}.txt`) using layered `-r requirements-runtime.txt` includes per cycle-36 design Q6. README install-section update follows. Cycle-35 L8 floor pin (`langchain-openai>=1.1.14`) must appear in `requirements-eval.txt`. *(Originally cycle-36; deferred per Q7=B to keep cycle-36 focused on strict-gate + cross-OS closure.)*

- `tests/` windows-latest CI matrix re-enable (cycle-37) — cycle 36 closed the strict-gate BACKLOG entry on ubuntu-latest single-OS but PARTIALLY closes the cross-OS portability entry. `windows-latest` GHA runner exhibits a SECOND hang at `threading.py:355` after the cycle-23 multiprocessing spawn-bootstrap test was skipif'd. Pytest reaches ~position 1168 in collection order (1157 passed + 11 skipped) before the threading interrupt fires. Cycle 37 should: (1) identify the threading-related test that hangs (likely in `test_cycle23_workflow_e2e.py` or `test_cycle23_rebuild_indexes.py`); (2) apply targeted skipif marker OR fix the test root cause; (3) re-enable matrix `[ubuntu-latest, windows-latest]` with `strategy.fail-fast: false`. Local Windows full suite passes 2985 + 20 skipped — only the GHA Windows runner exhibits this hang. *(Cycle-36 spawn entry; cycle-36 commit-4 reverted matrix to ubuntu-only after windows hang surfaced in commit-3 strict-gate run.)*

- `tests/` GHA-Windows multiprocessing spawn investigation (cycle-37) — cycle 23's `test_cross_process_file_lock_timeout_then_recovery` hangs on the GHA `windows-latest` runner at `popen_spawn_win32.py:112` (parent's `child.start()` blocks waiting on the spawn-bootstrap pipe). Local Windows pass time is 1.03s. Cycle 36 AC2 skipped the test with `skipif(os.environ.get("CI") == "true")` to unblock the strict-gate. Cycle 37 should reproduce on a self-hosted Windows runner, instrument the child-spawn pipe, and identify the divergence (likely editable-install pth resolution / `PYTHONNOUSERSITE` / `kb.config` PROJECT_ROOT heuristic in spawned child). Once fixed, narrow the skipif to `GITHUB_ACTIONS` only or remove it entirely. *(Cycle-36 spawn entry from AC2 / Step-5 Q1.)*

- `tests/_helpers/api_key.py` `mock_scan_llm` POSIX reload-leak investigation (cycle-37) — cycle 36 ubuntu-probe surfaced 10+ test failures in `test_capture.py` and `test_mcp_core.py` where `mock_scan_llm(...)` is installed but the real Anthropic SDK call goes through anyway and fails with 401 auth error from the dummy CI key. The mock plumbing patches `kb.capture.call_llm_json` but POSIX full-suite ordering may bypass the patch via reload-leak (cycle-19 L2 / cycle-20 L1 class). Cycle 36 marked the affected tests with `requires_real_api_key()` skipif (skip on CI dummy key, run locally on real key). Cycle 37 should investigate the reload chain that breaks the mock and either (a) widen the patch to all owners, (b) move the patch to autouse fixture with explicit reload-clear, or (c) refactor `kb.capture` to use a single SDK injection point. Affected tests: `tests/test_capture.py::TestCaptureItems::*`, `TestPipelineFrontmatterStrip::test_frontmatter_stripped_for_capture_source`, `TestRoundTripIntegration::test_capture_then_ingest_renders_wiki_summary`; `tests/test_mcp_core.py::TestKbCaptureWrapper::*`. *(Cycle-36 spawn entry from AC6 / Step-5 Q4.)*

- `tests/test_phase45_theme3_sanitizers.py::test_qb_symlink_outside_raw_rejected` POSIX symlink security gap (cycle-37) — cycle 36 ubuntu-probe surfaced REAL production security failure: `pair_page_with_sources` in `src/kb/review/context.py` resolves symlinks inside `raw/` without verifying the target stays within `raw/`. Test was masked on Windows by cycle-23 multiprocessing-hang; first-time POSIX run shows path traversal via symlink succeeded. Cycle 36 marked the test `skipif(os.name != "nt")` to unblock strict-gate. Cycle 37 should fix the production code: add `target.resolve().is_relative_to(raw_dir.resolve())` containment check before reading symlinked content. *(Cycle-36 spawn entry; real production POSIX security bug.)*

- `tests/test_capture.py::TestExclusiveAtomicWrite + TestWriteItemFiles` POSIX cleanup behaviour (cycle-37) — cycle 36 ubuntu-probe surfaced 4 test failures: `test_cleans_up_reservation_on_inner_write_failure`, `test_cleans_up_on_keyboard_interrupt`, `test_creates_dir_if_missing`, `test_pre_existing_file_collision`. The first two monkeypatch `kb.capture.atomic_text_write` to raise but the `pytest.raises` expectation isn't met on POSIX (the monkeypatch may not reach the production path). The latter two have POSIX-incompatible assertions (e.g., expected slug `decision-foo-2` becomes `decision-foo-3` on POSIX — extra file collision). Cycle 36 marked these `skipif(sys.platform != "win32")`. Cycle 37 should investigate whether the production `_exclusive_atomic_write` helper has POSIX-specific code paths that bypass the patched `atomic_text_write`, and whether tmp-dir cleanup ordering on POSIX produces the off-by-one slug counter. *(Cycle-36 spawn entry from AC11 probe.)*

- `Dependabot pip-audit drift on litellm GHSA-r75f-5x8p-qvmc` (cycle-37) — Dependabot reports the critical advisory (LiteLLM Proxy code execution via vulnerable proxy endpoints; fix=1.83.7 BLOCKED by `click<8.2` transitive constraint vs our `click==8.3.2` pin) but `pip-audit` on the live CI install env does not emit this ID as of 2026-04-26. Workflow `--ignore-vuln` excludes this (intentionally — pip-audit doesn't see it). Documented in cycle-36 investigation; monitor for pip-audit data refresh; escalate if pip-audit catches up (would force CI fail unless we add the ID to `--ignore-vuln`). *(Cycle-36 spawn entry from Step-5 Q17.)*

- `Dependabot pip-audit drift on litellm GHSA-v4p8-mg3p-g94g` (cycle-37) — Dependabot reports the high-severity advisory (LiteLLM authenticated MCP-stdio command execution; created 2026-04-25T23:37Z; fix=1.83.7 BLOCKED by same `click<8.2` transitive). Pip-audit on live CI install env does not emit the ID as of 2026-04-26. Same handling as `GHSA-r75f-5x8p-qvmc` above. *(Cycle-36 spawn entry from Step-5 Q17.)*
