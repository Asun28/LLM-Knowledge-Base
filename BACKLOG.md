...
- `compile/compiler.py` `load_manifest` has no value type check (cycle-83 threat T4) — `find_changed_sources` calls `stored.startswith("failed:")` on whatever `json.loads` produced. A hand-edited or corrupted `.data/hashes.json` holding a non-string value (int, list, null) raises `AttributeError` and kills the entire compile scan rather than skipping the bad row. Cycle 83's `_claims_content` guards the ingest-side read; the `compiler.py` read sites remain unguarded. (fix: guard with `isinstance(stored, str)` at the read sites and treat non-string values as "changed" so the source is re-ingested and the bad row self-heals.)
...
- `ingest/pipeline.py` `_persist_contradictions` silently drops a write when the lock times out (cycle-84 review MINOR) — `file_lock`'s default `LOCK_TIMEOUT_SECONDS` is 5.0s; if a holder is descheduled for longer, the waiting caller raises `TimeoutError`, which `_persist_contradictions` swallows. The contradiction write is lost with no error surfaced to the caller and no `contradictions.md` entry — the operator has no signal that a detected contradiction was discarded. Cycle 84 widened the deadline in the two threaded TESTS (they were failing under load), which means the production 5s default and this swallow path are no longer exercised by them. (fix: surface the drop — log at WARNING with the source_ref and claim count, and/or retry once with a longer deadline before giving up; then add a test that pins the timeout path with a deliberately-held lock and asserts the warning.)

- `tests/` contradiction lock tests do not reliably falsify a REMOVED lock (cycle-84 review MINOR) — `test_concurrent_identical_claim_block_dedups_once` can pass with a no-op lock because both writers may replace the file with identical bytes and still leave exactly one claim. The distinct-claim variants only fail if the two threads' read-modify-write windows actually overlap; the barrier aligns thread STARTS but does not guarantee overlap. Pre-existing weakness, improved but not closed by cycle 84's barrier. (fix: make overlap deterministic — patch the in-lock read to block on a `threading.Event` the other thread sets, so the second writer is provably inside its RMW window before the first commits; then confirm a no-op-lock variant loses a write.)

- `ingest/pipeline.py` human wiki-log line precedes the manifest commit (cycle-83 R2 MINOR) — `_run_ingest_body` appends the `Ingested …` line to `wiki/log.md` (step 7) before Design C's `_commit_ingest_manifest` runs (after the body returns). If the commit then fails, `wiki/log.md` shows a success-looking audit line for an ingest the manifest never recorded. The authoritative correlation surface (`.data/ingest_log.jsonl` `stage=success`) is correctly emitted only after the commit, so this is a human-log/machine-log inconsistency, not a data-loss issue. A narrow related window: an asynchronous `KeyboardInterrupt` landing after the commit but before the `stage=success` emission enters the outer `except BaseException` and emits `stage=failure` for an already-committed ingest. (fix: move the human success log to after the commit in `ingest_source`; and split pre-commit failure handling from post-commit interruption handling so a post-commit interrupt does not emit `failure`.)

- `utils/io.py` `atomic_json_write` does not fsync the parent directory (cycle-83 threat T12) — the temp file is fsynced before `os.replace`, but the directory entry is not, so a power loss immediately after a manifest write can revert the rename on some filesystems. Durability gap, distinct from cycle-83's crash-atomicity work. Low priority: the window is small and the consequence is a re-ingest, not corruption. (fix: `os.open(parent, O_DIRECTORY)` + `os.fsync` on POSIX; no-op on Windows.)
