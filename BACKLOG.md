...
- `tests/` coverage-visibility — 5 test files remain in the `test_v0NNN_*` naming pattern (cycle 93 folded the 4 `test_v091N_phase39N` files + the last 3 `test_phase4_audit_*` files — 7 files, 134 tests → 53 placements across 20 canonical receivers; 12 → 5. Cycle 92 folded the v0915 series 18 → 12; cycle 90 folded the recovered set 20 → 18). Verifying canonical-module coverage requires grepping versioned files. Freeze-and-fold cadence in progress. Per-cycle progress is in CHANGELOG-history.md, not here. (R3)
