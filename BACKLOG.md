...
- `ingest/pipeline.py` state-store fan-out — a single `ingest_source` mutates summary page, N entity pages, N concept pages, `index.md`, `_sources.md`, `.data/hashes.json`, `wiki/log.md`, `wiki/contradictions.md`, plus N `inject_wikilinks` writes. Every step is independently atomic, **none reversible**. (R2)
  **Cycle 83 closed the data-loss half.** The manifest reservation is now written as `in_progress:{hash}` rather than the bare hash, so a crash can no longer leave the manifest asserting a source is ingested when the wiki holds zero or partial pages — `find_changed_sources` re-selects any marker-valued entry, so the source is retried instead of skipped forever. Note the original entry text understated the window: the reservation happens BEFORE the first page write, not between manifest-write and log-append, so the exposure spanned the whole ingest body. The receipt-file remedy proposed here was evaluated and rejected — it records that a crash happened but leaves `manifest[K] = H` intact, so the permanent skip is not actually fixed.
...
- `ingest/pipeline.py` Phase-2 manifest confirmation ignores the caller's `manifest_path` (cycle-83 discovery) — `_run_ingest_body`'s Phase-2 write hardcodes `file_lock(HASH_MANIFEST)` / `load_manifest()` / `save_manifest()` with no path argument, while Phase-1 `_check_and_reserve_manifest` accepts a `manifest_path` parameter. Harmless in production (both resolve to the same file) but silently divergent under `compile_wiki(manifest_path=custom)`: compile writes markers to the custom path while ingest writes hashes to the module-level default. Consequence for test authors — any marker-lifecycle regression test that passes `manifest_path=` observes only `compile_wiki`'s cycle-25 marker and stays green under revert (vacuous). (fix: thread the owning path through `_run_ingest_body` and use it at both phases; audit existing `manifest_path=`-passing tests for vacuity at the same time.)

- `ingest/pipeline.py` liveness-aware duplicate detection for stale markers (cycle-83 threat T8) — `_claims_content` treats `in_progress:{hash}` as claiming the content, which is required to preserve the Phase 4.5 Q_A concurrent-ingest guarantee (`test_duplicate_content_concurrent_ingest`). A HARD-KILLED ingest leaves that marker behind, so a genuinely different file with identical content is skipped until the crashed source is retried. Self-healing (the marker never equals the current hash, so the crashed source is re-selected on the next compile and its pages get written), but the window is unbounded if the user never recompiles. Closing it needs to distinguish a live marker from a stale one. Constraint: cycle-83 threat T13 requires the manifest value stay a plain string prefixed `in_progress:` — a dict payload breaks `stored.startswith` in every version that reads the manifest. (fix: consider `in_progress:{hash}:{pid}:{started_at}` with `_pid_exists`-based classification, or a sidecar liveness file; treat the marker as claiming ONLY while the owning process is confirmed alive.)
...
- `compile/compiler.py` `in_progress:` markers are immortal (cycle-83 threat T6) — marker-valued entries are exempt from BOTH prune sites so cycle-25 AC7's "operator decides" contract holds. Cycle 83 widened the producer set from `compile_wiki` only to every `ingest_source` caller, so hard-killed ingests now accumulate markers from the CLI and MCP paths too, and AC7's warning enumerates every stale key by name — a repo with many stale markers gets an unbounded log line. (fix: add an age field or a sidecar `.data/marker_ages.json`, cap the enumerated list at N with an "and M more" suffix, and offer an explicit `kb compile --clear-stale-markers` rather than auto-deletion, which cycle-25 Q10 rejected for racing a live compile.)

- `tests/` contradiction-concurrency tests are load-sensitive (cycle-83 discovery) — `tests/test_ingest.py::test_persist_contradictions_concurrent` and `tests/test_cycle8_contradictions_idempotent.py::test_concurrent_same_day_same_source_distinct_claims_both_persist` both spawn two threads that contend on `file_lock(contradictions.md)` and assert both payloads land. They pass reliably in isolation (5/5) but failed intermittently — one or the other, roughly 3 runs in 4 — once cycle 83 added twelve tests that each ran a full `ingest_source` including the tail `rebuild_vector_index`. Proven to be load, not logic: the suite was green with cycle-83's production changes in place and only the new test file excluded, and neither failing test calls any changed code. Mitigated in cycle 83 by passing `_skip_vector_rebuild=True` in the new fixture, but the underlying fragility is untouched — the next cycle that adds heavy suite I/O will hit it again, and it will be a confusing failure because the failing test has no relationship to the change. (fix: give both tests a deterministic barrier — have each thread signal an `threading.Event` once it holds the lock so the other's attempt fully resolves before assertions run — rather than relying on wall-clock overlap; mirrors the cycle-82b MINOR-3 fix that made the `page_lock` exclusion tests event-driven. Alternatively raise `LOCK_TIMEOUT_SECONDS` for these two tests only.)
