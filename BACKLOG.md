...
- `tests/` coverage-visibility — 12 test files remain named `test_v0NNN_taskNN.py` / `test_v0NNN_phaseNNN.py` / `test_phase4_audit_*.py` (cycle 92 folded the complete remaining v0915 task series — 6 files, 204 tests → 17 receiver placements across 14 canonical files; 18 → 12. Cycle 90 folded the 12-file recovered cycle-59/62/63 set; 20 → 18). Verifying canonical-module coverage requires grepping versioned files. Freeze-and-fold cadence in progress; remaining files still to fold across future cycles. Per-cycle progress is in CHANGELOG-history.md, not here. (R3)
