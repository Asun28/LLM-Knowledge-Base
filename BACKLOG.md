...
- `query/embeddings.py` vector-index lifecycle — Phase 4.5 HIGH cycle 1 shipped H17 hybrid (mtime-gated rebuild + batch skip). Remaining deferred sub-items: (2) cold-load latency (0.8s+67MB on first query), (3) dim-mismatch auto-rebuild (stored embedding dim vs current model triggers automatic rebuild instead of silent BM25 fallback). Sub-item (1) atomic temp-DB-then-replace rebuild SHIPPED cycle 24 (AC5/AC6/AC8 — `os.replace` on `<vec_db>.tmp` with cache-pop+close before replace + crash-cleanup). Sub-item (4) `_index_cache` cross-thread lock symmetry shipped incrementally across cycles 3/6/24 (cycle 3 H8 added `_index_cache_lock` double-check; cycle 6 added `_conn_lock`; cycle 24 added explicit `_conn.close()` before `os.replace`). Bundle sub-items (2) + (3) into a dedicated vector-index cold-load cycle.

- `compile/compiler.py` `rebuild_indexes` `.tmp` awareness — Cycle 23 `rebuild_indexes` unlinks the vector DB but not a potential `<vec_db>.tmp` sibling from a crashed rebuild. Cycle-24 AC6 cleans `.tmp` at next `rebuild_vector_index` entry (belt-and-suspenders), so this is a hygiene improvement rather than a correctness gap. Low severity. *(Surfaced 2026-04-23 cycle 24 R2 Codex design eval.)*
  (fix: extend `rebuild_indexes` to also `tmp_path.unlink(missing_ok=True)`.)
...
- `utils/io.py` `file_lock` fair-queue / starvation under N-waiter contention — Cycle 24 AC9 added exponential backoff (floor 10ms, cap `LOCK_POLL_INTERVAL`) but did not add fair queueing; waiters can be out-raced by newer entrants. Residual under sustained contention. *(Surfaced 2026-04-23 cycle 24 threat model T3 residual.)*
...
- `lint/fetcher.py` `diskcache==5.6.3` — CVE-2025-69872 (GHSA-w8v5-vhqr-4h9v): pickle-deserialization RCE in diskcache cache files. No patched upstream version as of 2026-04-23 (Re-checked 2026-04-23 per cycle-24 AC12: `pip index versions diskcache` shows 5.6.3 = LATEST INSTALLED; `pip-audit --format=json` reports empty `fix_versions` for the CVE).
...
- `requirements.txt` `ragas==0.4.3` — CVE-2026-6587 (GHSA-95ww-475f-pr4f): server-side request forgery in `_try_process_local_file` / `_try_process_url` of `ragas.metrics.collections.multi_modal_faithfulness.util`. No patched upstream release as of 2026-04-23 (Re-confirmed 2026-04-23 per cycle-24 AC13: `pip-audit` reports empty `fix_versions`; `pip index versions ragas` shows 0.4.3 = LATEST INSTALLED; vendor did not respond to disclosure — identical no-upstream-fix profile to diskcache). ragas is a dev-eval-only dep (used manually for evaluation harness work); `grep -rnE "ragas|Ragas" src/kb` confirms zero runtime imports. Re-check on the next cycle's Step-2 baseline.
