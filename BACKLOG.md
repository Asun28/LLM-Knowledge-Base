...
- `query/embeddings.py` vector-index lifecycle — Phase 4.5 HIGH cycle 1 shipped H17 hybrid (mtime-gated rebuild + batch skip). Remaining deferred sub-items: (2) cold-load latency (0.8s+67MB on first query), (3) dim-mismatch AUTO-rebuild (stored embedding dim vs current model triggers automatic rebuild instead of silent BM25 fallback). Cycle-25 AC3/AC4/AC5 shipped the *observability* variant of sub-item (3): `VectorIndex.query` now logs operator-actionable remediation (`kb rebuild-indexes --wiki-dir <path>`) on mismatch, and a module-level `_dim_mismatches_seen` counter + `get_dim_mismatch_count()` getter expose event counts for diagnostic scripts. Sub-item (1) atomic temp-DB-then-replace rebuild SHIPPED cycle 24 (AC5/AC6/AC8 — `os.replace` on `<vec_db>.tmp` with cache-pop+close before replace + crash-cleanup). Sub-item (4) `_index_cache` cross-thread lock symmetry shipped incrementally across cycles 3/6/24. Remaining true-deferred: auto-rebuild (needs `VectorIndex` to hold `wiki_dir` or callback) + cold-load warm-load hook. Bundle into a dedicated vector-index cold-load cycle.
