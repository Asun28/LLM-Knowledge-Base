...
- CLI ↔ MCP parity — `cli.py` exposes 14 commands (cycle-27 shipped `search` / `stats` / `list-pages` / `list-sources` via AC1-AC4 with function-local imports of `kb.mcp.browse` + `kb.query.engine.search_pages` + new `_format_search_results` helper — `kb search` honours `--wiki-dir`; `kb stats` honours `--wiki-dir`; list-* intentionally omit it pending an MCP signature change). MCP exposes 28 tools. Remaining gap ≈ 14 (write-path tools `kb_review_page`/`kb_refine_page`/`kb_query_feedback`/`kb_save_source`/`kb_save_lint_verdict`/`kb_create_page`/`kb_capture`/`kb_save_synthesis`; deeper browse/health tools `kb_read_page`/`kb_verdict_trends`/`kb_graph_viz`/`kb_detect_drift`/`kb_reliability_map`/`kb_affected_pages`/`kb_lint_deep`/`kb_lint_consistency`). Structured `--format=json` output across both surfaces still open. (R2)
