...
- `tests/` windows-latest CI matrix re-enable (cycle-39+) — cycle 36 closed the strict-gate on ubuntu-latest single-OS; windows-latest GHA runner exhibits a SECOND hang at `threading.py:355` after cycle-23 multiprocessing spawn-bootstrap test was skipif'd. Pytest reaches ~position 1168 in collection order (1157 passed + 11 skipped) before the threading interrupt fires. Cycle 39+ should: (1) identify the threading-related test that hangs (likely in `test_cycle23_workflow_e2e.py` or `test_cycle23_rebuild_indexes.py`); (2) apply targeted skipif marker OR fix the test root cause; (3) re-enable matrix `[ubuntu-latest, windows-latest]` with `strategy.fail-fast: false`. Local Windows full suite passes 3003 + 11 skipped post-cycle-38 — only the GHA Windows runner exhibits this hang. *(Cycle-36 spawn entry; cycle-37+38 deferred per cycle-36 L1 — separate cycle for the new CI dimension to avoid extending the failed-CI-run pattern.)*

- `tests/` GHA-Windows multiprocessing spawn investigation (cycle-39+) — cycle 23's `test_cross_process_file_lock_timeout_then_recovery` hangs on the GHA `windows-latest` runner at `popen_spawn_win32.py:112` (parent's `child.start()` blocks waiting on the spawn-bootstrap pipe). Local Windows pass time is 1.03s. Cycle 36 AC2 skipped the test with `skipif(os.environ.get("CI") == "true")` to unblock the strict-gate. Cycle 39+ should reproduce on a self-hosted Windows runner, instrument the child-spawn pipe, and identify the divergence (likely editable-install pth resolution / `PYTHONNOUSERSITE` / `kb.config` PROJECT_ROOT heuristic in spawned child). Once fixed, narrow the skipif to `GITHUB_ACTIONS` only or remove it entirely. *(Cycle-36 spawn entry from AC2 / Step-5 Q1; cycle-37+38 deferred — investigation-heavy, requires self-hosted runner.)*

- `tests/test_capture.py::TestWriteItemFiles` POSIX off-by-one + creates_dir investigation (cycle-39+) — cycle 36 ubuntu-probe surfaced 2 test failures: `test_creates_dir_if_missing`, `test_pre_existing_file_collision`. The latter expected slug `decision-foo-2` becomes `decision-foo-3` on POSIX (extra file collision); the former has unclear POSIX divergence in `_write_item_files` mkdir path. Cycle 38 AC7+AC8 attempted investigation but scope-cut to cycle-39 per design M1 standing pre-auth — root-cause investigation requires direct POSIX shell access to instrument `_scan_existing_slugs` / `_build_slug` / `_reserve_hidden_temp` and trace the divergence. Cycle 38 retains the `@_WINDOWS_ONLY` skipif on these 2 tests. The other 2 cleans_up tests in TestExclusiveAtomicWrite were resolved in cycle 38 AC6 (dual-site atomic_text_write patch). *(Cycle-36 spawn entry from AC11 probe; cycle-37+38 deferred — POSIX investigation needs deeper access.)*
