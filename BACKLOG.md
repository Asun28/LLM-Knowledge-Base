...
- `mcp/quality.py` `kb_refine_page` accepts a body shorter than the page it replaces, with no guard (cycle-97 R1 DeepSeek P2) — the only size check is `len(updated_content) > MAX_INGEST_CONTENT_CHARS`, an UPPER bound. There is no comparison against the stored page, so a caller that supplies a 1 KB body for a 100 KB page silently destroys 99 KB. The write itself is atomic and locked (cycle 81/82), which makes this the opposite of a race: it is a correct write of wrong content. **Cycle 97 makes it more reachable, which is why it is filed here rather than left implicit.** A model that reads a page through `kb_review_page` may now receive a TRUNCATED body, and the mitigation shipped in cycle 97 is a notice outside the `wrap_wiki_context` fence — a warning label, not an enforcement point. Nothing downstream refuses a write derived from a truncated read. (fix: the smaller half is to thread the truncation fact through — `kb_review_page` marks the response, and `refine_page` refuses a body shorter than the stored page when the caller cannot show it saw the whole thing. The general half is a shrink guard with an explicit `force` / `confirm_deletion` parameter, which needs a threshold decision (percentage? absolute?) and has to stay compatible with legitimate large deletions, so it is its own cycle. Do not ship a bare percentage heuristic without deciding what a legitimate rewrite looks like.)

- `tests/` coverage-visibility — the R3 pattern set (`test_v0NNN_taskNN` / `test_v0NNN_phaseNNN` / `test_phase4_audit_*` and successors) is COMPLETE as of cycle 94 (24 → 0 across cycles 80/90/92/93/94; per-cycle detail in CHANGELOG-history.md). Remaining, downgraded in urgency: (a) opportunistic folds for the wider non-pattern versioned inventory (`test_v4_11_*` formats family, `test_v5_lint_augment_{cli,fetcher,orchestrator}`, `test_fixes_v0{50,60}`, `test_tier{1,2,3}_*_v09*`, `test_lint_query_fixes_v092`, `test_compiler_mcp_v093`, `test_lint_fix_v093`, etc.) — fold when a cycle touches the owning module, not as dedicated cycles; (b) enable `coverage` in CI and surface per-module % in PR comments. (R3) A wider versioned inventory sits outside this naming pattern (`test_v4_11_*` formats family, `test_v5_lint_augment_{cli,fetcher,orchestrator}`, `test_fixes_v0{50,60}`, `test_tier{1,2,3}_*_v09*`, etc.) — fold opportunistically when a cycle touches the owning module. The `archive/cycle-59|61|62` tags (fold-diff provenance for the now-landed recovered set) are no longer needed for replay; cycle 90 confirmed the archived diffs predate cycles 65-89 and must not be replayed verbatim.
...
- `lint/semantic.py` `build_completeness_context` has no production caller (cycle-97 design gate) — it was added in cycle 73 AC01 as the same-class peer of `build_fidelity_context` and is maintained alongside it (cycle-96 `_cap_page_content`, cycle-97 page-truncation notice) but nothing reaches it: `kb_lint_deep` and `kb lint-deep` call `build_fidelity_context` only. Documented as an orphan since cycle 71 (`docs/superpowers/decisions/2026-05-09-cycle-71-design.md:35,48`). Either wire it into the deep-lint path as a second pass (completeness and fidelity ask opposite questions — "is every claim sourced" vs "is every source claim represented") or delete it and stop paying peer-maintenance on dead code. Not urgent: it is tested, and the maintenance cost is one extra edit per paired-context cycle.
