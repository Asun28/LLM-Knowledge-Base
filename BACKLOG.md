...
- `pyproject.toml:29` + `lint/fetcher.py:51` httpx constraint mismatch — `pyproject.toml` specifies `httpx>=0.27` but `fetcher.py:51` asserts httpx 0.28.x at runtime; installs that satisfy the package metadata constraint but land on 0.27.x fail in production without any pip-install warning.
  (fix: tighten `pyproject.toml` to `httpx>=0.28,<0.29` to match the runtime assertion; or make the `fetcher.py` guard a logged warning rather than a hard failure; audit remaining `>=X.Y` lower-bounds in `pyproject.toml` for similar constraint/runtime divergence.)
...
- `requirements.txt` resolver conflicts (cycle-34 AC52 follow-up) — `pip check` reports four known conflicts that CI accepts via `continue-on-error: true`: (a) `arxiv 2.4.1` requires `requests~=2.32.0` but installed `requests==2.33.0`; (b) `crawl4ai 0.8.6` requires `lxml~=5.3` but installed `lxml==6.1.0`; (c) `instructor 1.15.1` requires `rich<15.0.0,>=13.7.0` but installed `rich==15.0.0`; (d) `dspy` reports `litellm` as missing (litellm is installed but not visible to dspy's resolver). Each has a known runtime workaround (none of `arxiv`/`crawl4ai`/`instructor`/`dspy` is imported by `src/kb/`). When upstream packages relax these constraints, drop the `continue-on-error: true` directive.
...
- `compile/compiler.py:603` `auto_publish_after_compile` exceptions swallowed — publish exceptions are caught, logged, and suppressed; `compile_wiki` reports success even when publish fails. A CI or release pipeline cannot distinguish "compiled + published" from "compiled but publish silently dropped".
  (fix: add `KB_STRICT_PUBLISH=1` env var; when set, re-raise publish exceptions instead of swallowing; document in `kb compile --help` and CI job step.)

- `mcp/core.py:383` + `query/engine.py:1077` prompt-injection boundary gap — wiki context is injected into synthesis prompts without a hardened instruction boundary; adversarial instructions in ingested `raw/` web content can propagate through the query/MCP response surface. `ingest/extractors.py:326` already marks source text as untrusted at ingestion time but the downstream query path lacks the same boundary.
  (fix: wrap wiki-context blocks with an XML-style fence (`<wiki_context>…</wiki_context>`) and add a system-prompt assertion that enclosed content is data, not instructions; apply consistently to `kb_query`, `kb_search`, and any MCP tool that surfaces wiki text.)

- `README.md` + `config.py:43` package-install `KB_PROJECT_ROOT` bootstrap undocumented — `kb.config` resolves project root from `cwd` → `pyproject.toml` walk → installed-file location; a `pip install kb` CLI silently resolves data paths into the venv unless `KB_PROJECT_ROOT` is set. README does not prominently document this requirement.
  (fix: add a "Non-clone install" section to README making `export KB_PROJECT_ROOT=/path/to/your/kb` a required first step; or ship `kb init <dir>` that writes a `.kb_root` sentinel and prints the env-var export command.)
