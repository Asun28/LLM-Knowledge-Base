...
- `utils/io.py` `_fsync_parent_dir` cannot report whether a flush actually happened (cycle-88 AC02 residual + cycle-88 R2 Codex P2) — it returns normally in THREE distinct cases that callers cannot tell apart: the flush succeeded; the platform is Windows (`os.name == "nt"` returns immediately); or the fsync failed with a tolerated errno (`EINVAL`/`ENOTSUP`/`EPERM`, or the directory could not be opened at all — some SMB and NFS mounts reject fsync on a directory handle). So `capture._finish_rollback` records `barrier_failed = False` and omits the `rollback_incomplete` marker even where no barrier occurred, and `durable_replace` / `durable_rename` likewise report success for a rename whose directory entry was never flushed. **This is deliberately NOT reported as indeterminate today** — the rollback/rename COMPLETED, and completion is a different axis from durability; firing the marker wherever a barrier is merely unavailable would make it constant on Windows and on those mounts, training the reader to ignore the one case it exists to flag. Both R1 (DeepSeek, Windows half) and R2 (Codex, POSIX-tolerated half) proposed reporting it; both remedies were rejected on that consistency ground and the observation filed here instead. The real fix is an API change — have the helper return a tri-state (`flushed` / `unsupported` / `skipped`) so each caller decides — which touches the two highest-traffic durability seams and wants its own cycle and review. Note there may be no cheap Windows answer at all: `DeleteFileW` has no write-through flag and `FlushFileBuffers` is unsupported on a directory handle (the volume-handle form flushes the whole volume and needs admin), so the honest outcome for that half may be "document the platform limit". (fix: tri-state return from `_fsync_parent_dir`; decide per call site whether unsupported warrants surfacing; write down the Windows finding either way.)
