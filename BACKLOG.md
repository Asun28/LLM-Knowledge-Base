...
- `tests/` coverage-visibility — 18 test files remain named `test_v0NNN_taskNN.py` / `test_v0NNN_phaseNNN.py` / `test_phase4_audit_*.py` (cycle 90 folded the complete 12-file recovered cycle-59/62/63 set — 69 tests → 8 canonical receivers — removing `phase4_audit_{ingest,query}` from this pattern set; 20 → 18). Verifying canonical-module coverage requires grepping versioned files. Freeze-and-fold cadence in progress; remaining files still to fold across future cycles. Per-cycle progress is in CHANGELOG-history.md, not here. (R3)
...
- `utils/io.py` Windows has no directory-flush primitive at all (cycle-89 AC01 residual) — cycle 89 closed the API half of this: `_fsync_parent_dir` now returns a `BarrierResult` tri-state (`FLUSHED` / `UNSUPPORTED` / `SKIPPED_PLATFORM`), so callers can finally tell a real flush from no flush, and `capture._finish_rollback` reports `barrier_unsupported` when a filesystem refuses. What remains is the underlying platform gap, which is NOT an API problem: on `nt` there is no cheap way to flush a directory entry at all. `DeleteFileW` has no write-through flag; `FlushFileBuffers` is unsupported on a directory handle (the volume-handle form flushes the whole volume and needs admin); transactional NTFS is deprecated. So `SKIPPED_PLATFORM` is currently permanent on Windows, and cycle 89 deliberately does NOT surface it per-call — it is constant there, so a note would ride along on every capture failure and train the reader to ignore it (the crying-wolf failure cycle 88 rejected twice). The honest outcome may well be won't-fix. (fix: confirm whether ANY unprivileged Win32 call makes a directory entry durable; if none exists, close this as won't-fix with the finding written into docs/reference/ so the next reviewer does not re-derive it — this is the third cycle in a row in which a reviewer has proposed reporting the Windows gap.)
