...
- `tests/test_review.py::test_embedding_dim_resolved` triple-escape-hatch sentinel upgrade candidate (cycle-56+) — cycle-55 R1 Sonnet MINOR observation: the test (folded from `test_v01011_review_feedback_fixes` per Q2 host-shape preservation C40-L5) has THREE escape hatches that combine to make it pass regardless of production state in the most likely future scenarios — (a) `if not hasattr(config, "EMBEDDING_DIM"): return  # Deleted — PASS`, (b) `try: from kb.query.embeddings import VectorIndex … except ImportError: pass`, (c) the `inspect.getsource(VectorIndex); assert "EMBEDDING_DIM" in src` is itself the same C11-L1 vacuous source-grep pattern this cycle explicitly upgraded elsewhere (AC1). Cycle-55 commit 384bbb4 docstring labeled it a "must-be-deleted-or-used" SENTINEL per cycle-15 L2 / cycle-44 L4 DROP-with-test-anchor — preserved verbatim. Upgrade options: (1) delete EMBEDDING_DIM from `kb.config` if truly unused (`grep -rnE "EMBEDDING_DIM" src/kb` returns config + VectorIndex.build only) and remove the test; (2) replace the getsource grep with a real validation pin (e.g. `VectorIndex(EMBEDDING_DIM=999, …).build()` raises a typed error). Filed per cycle-55 Step-20 R1 Sonnet MINOR + C40-L3 KNOWN-WEAK upgrade-candidate convention; same shape as the cycle-52 → cycle-53+ `test_prune_base_uses_canonical_rel_path_at_both_sites` candidate above.
