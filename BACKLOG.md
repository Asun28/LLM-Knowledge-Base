...
- `capture.py` rollback deletions have no durability barrier on Windows (cycle-88 AC02 residual) — cycle 88 added a single `_fsync_parent_dir(captures_dir)` after the all-or-nothing rollback deletions, but that helper returns immediately when `os.name == "nt"`, so on this project's PRIMARY development platform the deletions still get no barrier and a power loss can resurrect items the caller was told do not exist. Unlike the rename case there is no cheap Win32 equivalent to route around it: `DeleteFileW` has no write-through flag, and `FlushFileBuffers` is unsupported on a directory handle (the volume-handle form flushes the whole volume and needs admin). Deliberately scoped out rather than faked — cycle 87 exists because a docstring claimed NTFS durability that `os.replace` never provided, and repeating that on the delete path would be the same defect. The cross-platform half (the `rollback_incomplete` indeterminacy report) DID ship and is the more load-bearing one. (fix: investigate whether a POSIX-style guarantee is reachable at all on NTFS — e.g. transactional NTFS is deprecated, so the honest answer may be "document the platform limit and rely on the indeterminacy report"; if so, close this as won't-fix with the finding written down.)
