...
- `lint/verdicts.py` `load_verdicts` readers without lock — write-write race fixed (PR #17 added `_VERDICTS_WRITE_LOCK`); remaining: `get_page_verdicts`, `get_verdict_summary`, `kb_verdict_trends` all call `load_verdicts` WITHOUT holding any lock. Writer mid-rename via `atomic_json_write` can raise `PermissionError` on Windows; `load_verdicts` catches `json.JSONDecodeError` but NOT `PermissionError`/`OSError`, so a reader hitting the rename window aborts `kb_lint`/`kb_verdict_trends` with a raw exception. (R4)
  (fix: catch `(OSError, json.JSONDecodeError)` in `load_verdicts` with single retry after 50ms)
