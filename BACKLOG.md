...
- `lint/semantic.py:428` `build_completeness_context` `paired['page_content']` cap + wrap (deferred — file BACKLOG entry post-cycle-72) — direct same-class peer of cycle-72 AC01 (`build_fidelity_context` cap). The completeness function shares the same `paired["page_content"]` read pattern but has NO `wrap_wiki_context` fence at all (cycle-71 AC03 wrap landed only on `build_fidelity_context`). Cycle-72 deliberately scoped per threat-model §T1 OOS justification — design-decision §DIVERGENCE F-1 vs C1 picked DEFER over R2's expand-AC01 because doing F-1 properly = full cycle-71 AC03 + cycle-72 AC01 pair (≈+100 LoC + new wrap migration + threat re-eval), not the +50 LoC R2 framed. Cycle-73+ to apply AC01-shaped cap at L428 PLUS add `wrap_wiki_context` fence (analog of cycle-71 AC03) in a single AC.

- `kb.lint.verdict_db` `prompt_version` schema column (deferred — file BACKLOG entry post-cycle-72) — surfaced by cycle-72 threat-model §T7 Repudiation. Verdict-DB rows saved by `kb_save_lint_verdict` do NOT carry the prompt-shape version. Investigators of pre-cycle-72 verdicts cannot determine whether the LLM call used the OLD `<wiki_page_body>` / `<raw_source_N>` / `<untrusted_source>` literal sentinels or the cycle-70+ `<wiki_context>` fence. Forensic gap, not a runtime threat. Cycle-73+ to add `prompt_version: int` column with default `0` (= pre-cycle-70) and read-side back-fill for legacy rows; cycle-72-on writes stamp current version.

- Tier-boundary verifier — scan-tier output that triggers orchestrate-tier side effects (deferred — file BACKLOG entry post-cycle-72) — surfaced by cycle-72 threat-model §T8 EscalationOfPrivilege. Cycle-72 reduces the *probability* of successful prompt injection (T1-T6 entry point) but does NOT bound the *blast radius*. Scan-tier `_call_llm_json` outputs that propose `kb_create_page` / `kb_save_lint_verdict`-class side effects flow into orchestrate-tier callers without independent tier-boundary verification. Defense-in-depth complement to the cycle-7..cycle-72 prompt-fence family. Cycle-73+ to add an orchestrate-tier verifier call that re-gates scan-tier-proposed side effects against a stricter schema or a confirmation-prompt round-trip.
