...
- `mcp/quality.py` `kb_review_page` + `kb_lint_deep` unbounded source-read size (cycle-95 R1 P1 residual) — both build LLM context by reading every raw source referenced by their ONE page (`review/context.py:62`) with a source-count cap but no total-BYTES cap. Bounded by that page's `source:` list rather than by the corpus, so they were deliberately NOT moved onto the cycle-95 long-tool admission budget — a page with 3 sources is genuinely short. But a page citing a handful of very large raw files can still read tens of MB into a single tool response. Distinct from the tier-boundary caps (which bound the LLM's OUTPUT) and from `QUERY_CONTEXT_MAX_CHARS` (which bounds assembled query context).
  (fix: a per-call total read-byte budget in `build_review_context` / the deep-lint context builder, spending it across sources in order and appending an explicit truncation notice — mirror the cycle-71 `kb_read_page` footer-reservation pattern so the notice fits inside the cap.)
