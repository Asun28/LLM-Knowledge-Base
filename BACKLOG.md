...
- `ingest/pipeline.py:761-791` `_update_sources_mapping` and `pipeline.py:801-829` `_update_index_batch` — RMW concurrency residual. Both helpers read full file → mutate in memory → atomic-write WITHOUT holding `file_lock(target_path)`. Concurrent ingests may produce lost-update races even though serial re-calls dedup correctly (cycle-33 AC6+AC7+AC8 pinned the serial dedup-on-recall + merge-on-new-pages contracts via docstring + `atomic_text_write` spy regressions in `tests/test_cycle33_ingest_index_idempotency.py`; cycle-33 narrowed this entry from "duplicate entries on re-ingest" to concurrency-only since the documented + tested serial contract closes the named failure mode). (R2; cycle-33 narrowing)
  (fix: introduce an `IndexWriter` helper wrapping all four index-file writes — `_sources.md`, `index.md`, `_categories.md`, `log.md` — with documented order and per-file `file_lock`. Until then, callers MUST serialise ingest manually.)

- `utils/sanitize.py:30-31,68-75` — `_rel(Path(fn_str))` slash-normalises Windows path separators (`\\` → `/`) BEFORE the `_ABS_PATH_PATTERNS` regex sweep. The regex covers backslash UNC + long-path forms only at `sanitize.py:13-17`; after `_rel` runs on an outside-PROJECT_ROOT filename attribute carrying ordinary UNC `\\server\share\...`, the slash-normalised form bypasses the regex. (Long-path `\\?\C:\...` and UNC long-path `\\?\UNC\server\share\...` remain redacted because the drive-letter / long-path regex alternatives still match after slash-normalize.) Three xfail-strict / regular tests in `tests/test_cycle33_mcp_core_path_leak.py::TestSanitizeErrorTextUNCAndLongPath` document the gap. *(Surfaced 2026-04-25 cycle 33 R1 Codex review; spawn entry per Q8 design decision — opening cost of closing `mcp/core.py:762,881`.)*
  (fix: union `_ABS_PATH_PATTERNS` with forward-slash UNC variants (`/{2}[^\s'\"?/]+/[^\s'\"]+`) OR pre-normalize-check before `_rel(Path(fn_str))` substitution. When fix lands, removing the `pytest.mark.xfail(strict=True)` marker on `test_windows_ordinary_unc_filename_redacts` will force the marker removal automatically (strict=True semantics).)

- `ingest/pipeline.py:770-779` `_update_sources_mapping` empty-list semantics — when called with `wiki_pages=[]`, the function emits `- \`raw/path\` → \n` (a line with no page refs). Behaviour is undocumented; design call needed (skip-empty? error? emit empty?). *(Surfaced 2026-04-25 cycle 33 R1-08; deferred to follow-up.)*

- `ingest/pipeline.py:771,777,784` `_update_sources_mapping` backtick-in-`source_ref` lookup mismatch — `escaped_ref = source_ref.replace("\`", r"\\\`")` written but raw `source_ref` searched at lines 777 and 784. Repeat calls with backtick-bearing source_refs could duplicate. Low-risk because slugified MCP inputs cannot contain backticks (slugify excludes them per `utils/text.py`). *(Surfaced 2026-04-25 cycle 33 R1-10; deferred low-risk.)*

- `mcp/core.py:167-178,681-686,812-817` — `kb_ingest_content` and `kb_save_source` filename validation is weaker than `kb_query.save_as` (`core.py:188-216`). `_validate_file_inputs` only checks empty + length + content size; no ASCII / Windows-reserved-name / homoglyph checks. `slugify` preserves Unicode word chars (`utils/text.py:173`), so a homoglyph or NUL-byte filename can sneak past validation into the slug. *(Surfaced 2026-04-25 cycle 33 R1-11; deferred to a write-path input-validation cycle.)*
  (fix: factor the `_validate_save_as_slug` helper at `core.py:188-216` into a shared `_validate_filename_slug` and call from `_validate_file_inputs`, OR add a Windows-reserved + homoglyph + NUL guard step before slugify.)
...
- `requirements.txt` resolver conflicts (cycle-34 AC52 follow-up) — `pip check` reports three known conflicts that cycle 34's CI gate accepts via `continue-on-error: true` on the `pip check` step ONLY: (a) `arxiv 2.4.1` requires `requests~=2.32.0` but installed `requests==2.33.0`; (b) `crawl4ai 0.8.6` requires `lxml~=5.3` but installed `lxml==6.1.0`; (c) `instructor 1.15.1` requires `rich<15.0.0,>=13.7.0` but installed `rich==15.0.0`. Each has a known runtime workaround (none of `arxiv`/`crawl4ai`/`instructor` is imported by `src/kb/`). When upstream packages relax these transitive constraints, drop the `continue-on-error: true` directive to make `pip check` strict in CI. *(Surfaced 2026-04-25 cycle 34 Step 9; tracked per design-gate Q9 + threat-model T5.)*

- `docs/architecture/architecture-diagram*.html` v0.11.0 sync + PNG re-render (cycle-34 AC52 follow-up) — cycle 34 bumped version 0.10.0 → 0.11.0 across `pyproject.toml`, `src/kb/__init__.py`, `README.md` badge, and `CLAUDE.md` state line, but the architecture diagrams at `docs/architecture/architecture-diagram.html:501` and `architecture-diagram-detailed.html:398` still show v0.10.0. CLAUDE.md "Architecture Diagram Sync (MANDATORY)" rule requires the PNG to be re-rendered after every HTML edit. Cycle 34 deferred AC4e per design-gate NEW-Q15 fallback (option B "defer to cycle 35 with BACKLOG entry") given practical scope at 57 ACs / 18 file groups. *(Spawn entry from cycle 34 Step 9; close in cycle 35 with the Playwright re-render command per CLAUDE.md docs.)*
...
- `requirements.txt` split into per-extra files (cycle-36 follow-up) — cycle 34 AC2 declared `[project.optional-dependencies]` extras (`hybrid`/`augment`/`formats`/`eval`/`dev`) in `pyproject.toml`, but `requirements.txt` remains a full-dev superset. Cycle 36 (test-infrastructure cycle per the comprehensive review's 5-cycle plan) will split into `requirements-runtime.txt` / `requirements-hybrid.txt` / `requirements-augment.txt` / etc. so users running `pip install -r requirements-runtime.txt` get the minimum install path. *(Cycle-34 design Q10 explicitly deferred this to cycle 36 to avoid concurrent extras-declaration + pin-reshuffle blast radius.)*

- `tests/` strict full-pytest CI gate (cycle-36 follow-up) — cycle 34 bootstrapped CI but the `pytest -q` step is `continue-on-error: true` because the test suite has THREE classes of pre-existing CI-environment-fragility that surface even on `windows-latest`: (a) integration tests assuming a populated `wiki/` directory that is gitignored (e.g., `tests/test_cycle10_quality.py::test_kb_refine_page_surfaces_backlinks_error_on_failure` + `test_kb_affected_pages_surfaces_shared_sources_error_on_failure` fail with "Page not found: concepts/rag" because the CI repo has no wiki content); (b) tests requiring real ANTHROPIC_API_KEY for SDK calls the dummy key can't satisfy; (c) timing-sensitive rate-limit tests with 1-second precision boundaries that flake on slower CI runners (e.g., `test_over_cap_rejected_with_retry_after` asserting `3601 <= 3600`). Cycle 36 fixes: (i) seed minimal `wiki/` content via fixture or `tmp_kb_env` for tests that need it; (ii) mark Anthropic-SDK-instantiating tests with `pytest.mark.skipif(not has_real_api_key())`; (iii) add a `pytest.mark.flaky` reruns wrapper to the timing-sensitive tests, OR widen tolerance to ≥2 seconds. Then drop `continue-on-error: true` from the pytest step. *(Surfaced 2026-04-25 cycle 34 CI-rerun-3 on the windows-latest runner; full local Windows suite passes 2930 + 10 + 1 per Step 10 verification.)*

- `tests/` cross-OS portability (cycle-36 follow-up) — cycle 34 bootstrapped CI on `ubuntu-latest`, but the existing 2941-test suite was authored on Windows (developer env) and assumes Windows path semantics in ~22 tests: `tests/test_capture.py` (TestExclusiveAtomicWrite + TestWriteItemFiles + TestCaptureItems — anthropic SDK auth + tmp-dir creation order); `tests/test_cycle10_validate_wiki_dir.py::test_validate_wiki_dir_symlink_to_outside_rejected` (POSIX symlink semantics differ from Windows); `tests/test_cycle10_quality.py::test_kb_refine_page_surfaces_backlinks_error_on_failure` (Page-not-found shape); `tests/test_cycle11_utils_pages.py::test_page_id_normalizes_backslashes_to_posix_id` (Windows backslash → POSIX slash test fails on POSIX where `\` is treated as a literal filename character); `tests/test_phase45_theme3_sanitizers.py::test_qb_symlink_outside_raw_rejected` (symlink behaviour); `tests/test_v0915_task09.py::TestValidatePageIdAbsoluteCheck::test_absolute_path_rejected` (Windows-style absolute path detection). Cycle 34 worked around this by switching the CI workflow to `windows-latest` so the existing suite runs cleanly. Cycle 36 should: (a) add `pytest.mark.skipif(sys.platform != "win32", reason="...")` markers to the 22 OS-specific tests and re-enable `ubuntu-latest` CI for the platform-portable subset; (b) add a Linux-specific test variant set covering POSIX path semantics for the same contract; (c) optionally add a CI matrix `[ubuntu-latest, windows-latest]` for full cross-OS confidence. *(Surfaced 2026-04-25 cycle 34 CI-rerun-2 failure on the original `ubuntu-latest` workflow.)*
