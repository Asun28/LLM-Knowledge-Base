...
**Step-11 security verify:** all 10 threats from `2026-04-25-cycle31-threat-model.md` §7 IMPLEMENTED per mechanical-grep/AST scan in `docs/superpowers/decisions/2026-04-25-cycle31-security-verify.md`. No PARTIAL / MISSING. Same-class peer scan confirms `_is_mcp_error_response` is additive (7 occurrences: 1 def + 3 AC1-AC3 + 3 AC8) and no production behaviour change for the 5 non-retrofitted legacy wrappers (full-suite 2872 passed + 10 skipped, unchanged green).
